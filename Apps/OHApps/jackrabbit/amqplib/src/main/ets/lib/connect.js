...
import * as net from './compat/node/net';
import * as tls from './compat/node/tls';
...
var CLIENT_PROPERTIES = {
  "product": "amqplib",
  "version": '1.0.0',
  "platform": `OpenHarmony 4.0.0.600`,
...
function openFrames(vhost, query, credentials, extraClientProperties) {
  if (!vhost)
    ...
    vhost = decodeURIComponent(vhost)//QS.unescape(vhost);
...
function connect(url, socketOptions, openCallback) {
  ...
  if (typeof url === 'object') {
    ...
    var parts = new URL(url, true, undefined); // yes, parse the query string
...
export { connect, credentialsFromUrl }

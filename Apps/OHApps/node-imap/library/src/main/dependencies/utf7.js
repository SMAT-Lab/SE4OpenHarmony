...
var encode = function (str, mask) {
    ...
    return str.replace(regexes[mask], function (chunk) {
        // + is represented by an empty sequence +-, otherwise call innerEncode().
        return '+' + (chunk === '+' ? '' : innerEncode(chunk)) + '-';
    });
};

// RFC 2152 UTF-7 encoding with all optionals.
var encodeAll = function (str) {
    // We replace subsequent disallowed chars with their escape sequence.
...
let imap = {
    ...
    encode: function (str) {
        ...
        return str.replace(/&/g, '&-').replace(/[^\x20-\x7e]+/g, function (chunk) {
            // & is represented by an empty sequence &-, otherwise call innerEncode().
            chunk = (chunk === '&' ? '' : innerEncode(chunk)).replace(/\//g, ',');
            return '&' + chunk + '-';
        });
    },
    decode: function (str) {
        return str.replace(/&([^-]*)-/g, function (_, chunk) {
            // &- represents &.
            if (chunk === '') return '&';
            return innerDecode(chunk.replace(/,/g, '/'));
        });
    }

};

export { imap as default, encode, encodeAll, decode }
...
import { inherits, inspect, isDate } from '../polyfill/util';
import utf7 from '../dependencies/utf7';
import ImapError from './ImapError'
import process from '../polyfill/process';
...
Connection.prototype.connect = async function () {
    ...
    if (config.tls) {
        tempSocket = config.socket || socket.constructTLSSocketInstance();
        tlsOptions = config.tlsOptions;
    } else {
        tempSocket = config.socket || socket.constructTCPSocketInstance();
    }
    this._sock = tempSocket;
    console.log(`zdy---socket初始化--->` + this._sock)

    console.log(`zdy---获取本机ip--->`)
    let netHandle = await connection.getDefaultNet()

    if (!netHandle) {
        self.emit('error', new ImapError('get Default Net is fail'));
        return;
    }
    let netSyncInfo = await connection.getConnectionProperties(netHandle)
    ...
    || !netSyncInfo.linkAddresses[0] || !netSyncInfo.linkAddresses[0].address) {
        self.emit('error', new ImapError('get local ip is fail'));
        return;
    }
    console.log(`zdy---开始bind--->${{
        address: netSyncInfo.linkAddresses[0].address.address,
        port: netSyncInfo.linkAddresses[0].address.port,
        family: 1
    }}`)
    await this._sock.bind({
        address: netSyncInfo.linkAddresses[0].address.address,
        port: netSyncInfo.linkAddresses[0].address.port,
        family: 1
    })
    console.log(`zdy---bind成功--->`)
    await this._sock.setExtraOptions({
        keepAlive: true,
        socketTimeout: config.connTimeout
    })
    console.log(`zdy---setExtraOptions成功--->`)
    ...
    function onconnect() {
        ...
        self._tmrAuth = setTimeout(function () {
            var err = new ImapError('Timed out while authenticating with server');
    ...
    this._sock.on('connect', onconnect);
    this._onError = function (err) {
        clearTimeout(self._tmrConn);
        clearTimeout(self._tmrAuth);
        let tempErr = new ImapError(err.message)
        if (self.debug) {
            self.debug('[connection] Error: ' + tempErr);
        }
        tempErr.source = 'socket';
        self.emit('error', tempErr);
    };
    ...
    parser.on('continue', function (info) {
        ...
        if (type === 'IDLE') {
            ...
            && !self._idle.enabled) {
                ...
                if (self && self._config && self._config.tlsOptions) {
                    self._sock.send('DONE' + CRLF);
                } else {
                    self._sock.send({
                        data: 'DONE' + CRLF
                    });
    ...
    this._tmrConn = setTimeout(function () {
        var err = new ImapError('Timed out while connecting to server');
        err.source = 'timeout';
        self.emit('error', err);
        this._sock.close();
    }, config.connTimeout);
    console.log(`zdy---getAddressesByName--->`)
    let addressInfo = await connection.getAddressesByName(config.host)
    if (!addressInfo || addressInfo.length < 1) {
        self.emit('error', new ImapError('parse host fail'));
        return;
    }
    ...
    if (config.tls) {
        tlsOptions.address.address = parseHost;
        tlsOptions.address.port = config.port ? config.port : parsePort;
        console.log(`zdy---tls 连接开始--->${JSON.stringify(tlsOptions)}`)
        let connectInfo = await this._sock.connect(tlsOptions)
        console.log(`zdy---tls 连接成功--->`)
    } else {
        console.log(`zdy---tls 连接开始--->`)

        let tcpOption = {
            address: {
                address: parseHost,
                port: config.port ? config.port : parsePort,
                family: 1
            },
            timeout: config.connTimeout
        }
        console.log(`zdy---tcp 连接开始--->${JSON.stringify(tcpOption)}`)
        let connectInfo = await this._sock.connect(tcpOption)
        console.log(`zdy---tcp 连接成功--->`)
    }
};
Connection.prototype.getSupportList = function () {
    return this._caps;
};
...
Connection.prototype.append = function (data, options, cb) {
    ...
    if (!options.mailbox) {
        if (!this._box)
            throw new ImapError('No mailbox specified or currently selected');
        else
...
Connection.prototype.openBox = function (name, readOnly, cb) {
    ...
    cmd += ' "' + encname + '"';
    console.log(`zdy---openBox cmd--->${cmd}`)
...
Connection.prototype.status = function (boxName, cb) {
    if (this._box && this._box.name === boxName)
        throw new ImapError('Cannot call status on currently selected mailbox');
...
Connection.prototype.expunge = function (uids, cb) {
    ...
    if (uids !== undefined) {
        ...
        if (!this.serverSupports('UIDPLUS'))
            throw new ImapError('Server does not support this feature (UIDPLUS)');
...
Connection.prototype._store = function (which, uids, cfg, cb) {
    ...
    || (Array.isArray(items) && items.length === 0))
        throw new ImapError((isFlags ? 'Flags' : 'Keywords')
    ...
    for (var i = 0, len = items.length; i < len; ++i) {
        if (isFlags) {
            ...
            if (RE_INVALID_KW_CHARS.test(items[i])) {
                throw new ImapError('The keyword "' + items[i]
...
Connection.prototype._move = function (which, uids, boxTo, cb) {
    ...
    && this._box.flags.indexOf('\\Deleted') === -1) {
        throw new ImapError('Cannot move message: '
...
Connection.prototype._storeLabels = function (which, uids, labels, mode, cb) {
    ...
    || (Array.isArray(labels) && labels.length === 0))
        throw new ImapError('labels argument must be a string or a non-empty Array');
...
Connection.prototype._sort = function (which, sorts, criteria, cb) {
    if (this._box === undefined)
        ...
        throw new ImapError('Expected array with at least one sort criteria');
    else if (!Array.isArray(criteria))
        throw new ImapError('Expected array for search criteria');
    ...
    sorts = sorts.map(function (c) {
        if (typeof c !== 'string')
            throw new ImapError('Unexpected sort criteria data type. '
...
Connection.prototype._thread = function (which, algorithm, criteria, cb) {
    ...
    if (!this.serverSupports('THREAD=' + algorithm))
        throw new ImapError('Server does not support that threading algorithm');
...
Connection.prototype._resUntagged = function (info) {
    ...
    if (type === 'bye') {
        ...
        if (this._curReq && this._curReq.cb && typeof this._curReq.cb === 'function') {
            this._curReq.cb()
        }
        ...
        if (this.state === 'connected' && !this._curReq) {
            ...
            var err = new ImapError('Received negative welcome: ' + info.text);
            err.source = 'protocol';
            this.emit('error', err);
            this._sock.close();
        }
                ...
                || forceNoop)
                    ...
                    if (self._idle.started === undefined) {
                        ...
                        if (timeDiff >= idleWait) {
                            ...
                            if (self._config && self._config.tlsOptions) {
                                self._sock.send('DONE' + CRLF);
                            } else {
                                self._sock.send({
                                    data: 'DONE' + CRLF
    ...
    self._enqueue('CAPABILITY', function () {
        ...
        if (self.state === 'connected') {
            ...
            if (self.serverSupports('LOGINDISABLED')) {
                err = new ImapError('Logging in is disabled on this server');
            ...
            if (self.serverSupports('AUTH=XOAUTH') && self._config.xoauth) {
                ...
                err = new ImapError('No supported authentication method(s) available. '
                + 'Unable to login.');
                err.source = 'authentication';
                return reentry(err);
            }
...
/**
 * OpenHarmony暂不支持从tcpSocket升级到tlsSocket
 */
...
Connection.prototype._processQueue = function () {
    if (this._curReq || !this._queue.length || !this._sock)
        return;
    ...
    if (this.config && this.config.tlsOptions) {
        this._sock.send(out + CRLF);
    } else {
        this._sock.send({
            data: out + CRLF,
            encoding: 'utf8'
...
Connection.prototype._enqueue = function (fullcmd, promote, cb) {
    ...
    && this._idle.enabled) {
        ...
        if (this._idle.started > 0) {
            ...
            if (self.config && self.config.tlsOptions) {
                self._sock.send('DONE' + CRLF);
            } else {
                self._sock.send({
                    data: 'DONE' + CRLF
...
function validateUIDList(uids, noThrow) {
    for (var i = 0, len = uids.length, intval; i < len; ++i) {
        ...
        if (isNaN(intval)) {
            var err = new ImapError('UID/seqno must be an integer, "*", or a range: '
            + uids[i]);
            if (noThrow)
                return err;
            else
            ...
            var err = new ImapError('UID/seqno must be greater than zero');
            if (noThrow)
                return err;
            else
                throw err;
...
function buildSearchQuery(options, extensions, info, isOrChild) {
    ...
    for (var i = 0, len = options.length; i < len; ++i) {
        ...
        if (typeof criteria === 'string')
            ...
            throw new ImapError('Unexpected search option data type. '
            ...
            // -- Standard criteria --
                ...
                case 'TO':
                    if (!args || args.length !== 1)
                        throw new ImapError('Incorrect number of arguments for search option: '
                        + criteria);
                ...
                case 'SINCE':
                    if (!args || args.length !== 1)
                        ...
                        if ((args[0] = new Date(args[0])).toString() === 'Invalid Date')
                            throw new ImapError('Search option argument must be a Date object'
                ...
                case 'SMALLER':
                    ...
                    if (isNaN(num))
                        throw new ImapError('Search option argument must be a number');
                    searchargs += modifier + criteria + ' ' + args[0];
                    break;
                case 'HEADER':
            ...
            // Extensions ==========================================================
                ...
                case 'X-GM-THRID': // Gmail thread ID
                    if (extensions.indexOf('X-GM-EXT-1') === -1)
                        throw new ImapError('IMAP extension not available for: ' + criteria);
                    if (!args || args.length !== 1)
                        throw new ImapError('Incorrect number of arguments for search option: '
                        + criteria);
                    else {
                ...
                // http://tools.ietf.org/html/rfc3501#section-6.4.4
                    ...
                    if (!validateUIDList(seqnos, true)) {
                        ...
                        throw new ImapError('Unexpected search option: ' + criteria);
            }
        }
        if (isOrChild)
            break;

...
export class Camera {
  ...
  public m_width: number = 480;
  public m_height: number = 600;
...
//    this.m_ctx = context;
  ...
  public DrawSolidCircle(center: box2d.b2Vec2, radius: number, axis: box2d.b2Vec2, color: box2d.b2Color): void {
    ...
    if (ctx) {
      ...
      ctx.beginPath();
      ctx.arc(cx, cy, radius, 0, box2d.b2_pi * 2);
      ctx.moveTo(cx, cy);
  ...
  public DrawStringWorld(x: number, y: number, message: string): void {
    ...
    if (ctx) {
      ...
      p.y *= -1;
      //const cc: box2d.b2Vec2 = DebugDraw.DrawStringWorld_s_cc.Set(0.5 * ctx.width, 0.5 * ctx.height);
      const cc: box2d.b2Vec2 = DebugDraw.DrawStringWorld_s_cc.Set(0.5 * 480, 0.5 * 750);

...
var cases = [
  ...
  desc: '2_processing_instructions',
  ...
  desc: 'should_convert_comment',
  xml: '<!-- \t Hello, World! \t -->',
  js1: { "_comment": " \t Hello, World! \t " },
  js2: { "elements": [{ "type": "comment", "comment": " \t Hello, World! \t " }] }
}, {
  desc: 'should_convert_2_comments',
  xml: '<!-- \t Hello \t -->\n<!-- \t World \t -->',
  js1: { "_comment": [" \t Hello \t ", " \t World \t "] },
  js2: {
    "elements": [{ "type": "comment", "comment": " \t Hello \t " }, { "type": "comment", "comment": " \t World \t " }]
  }
}, {
  desc: 'should_convert_cdata',
  xml: '<![CDATA[ \t <foo></bar> \t ]]>',
  js1: { "_cdata": " \t <foo></bar> \t " },
  js2: { "elements": [{ "type": "cdata", "cdata": " \t <foo></bar> \t " }] }
}, {
  desc: 'should_convert_2_cdata',
  xml: '<![CDATA[ \t data]]><![CDATA[< > " and & \t ]]>',
  js1: { "_cdata": [" \t data", "< > \" and & \t "] },
  js2: { "elements": [{ "type": "cdata", "cdata": " \t data" }, { "type": "cdata", "cdata": "< > \" and & \t " }] }
}, {
  desc: 'should_convert_doctype',
  xml: '<!DOCTYPE note [\n<!ENTITY foo "baa">]>',
  js1: { "_doctype": "note [\n<!ENTITY foo \"baa\">]" },
  js2: { "elements": [{ "type": "doctype", "doctype": "note [\n<!ENTITY foo \"baa\">]" }] }
}, {
  desc: 'should_convert_element',
  xml: '<a/>',
  js1: { "a": {} },
  js2: { "elements": [{ "type": "element", "name": "a" }] }
}, {
  desc: 'should_convert_2_same_elements',
  xml: '<a/>\n<a/>',
  js1: { "a": [{}, {}] },
  ...
  desc: 'should_convert_text_in_element',
  xml: '<a> \t Hi \t </a>',
  js1: { "a": { "_text": " \t Hi \t " } },
  ...
  desc: 'should_convert_nested_elements',
  xml: '<a>\n\v<b/>\n</a>',
  js1: { "a": { "b": {} } },
  js2: { "elements": [{ "type": "element", "name": "a", "elements": [{ "type": "element", "name": "b" }] }] }
}, {
  desc: 'should_convert_3_nested_elements',
  xml: '<a>\n\v<b>\n\v\v<c/>\n\v</b>\n</a>',
  js1: { "a": { "b": { "c": {} } } },
  js2: {
    "elements": [{
      "type": "element",
      "name": "a",
      "elements": [{ "type": "element", "name": "b", "elements": [{ "type": "element", "name": "c" }] }]
    }]
  }
}
];

var tests = function (direction, options) {
  var i, tests = [];
  options = options || {};
  ...
  function applyOptions(obj, pathKey) {
    ...
    if (obj instanceof Array) {
      ...
      for (key in obj) {
        ...
        if (
          options.compact &&
          (helper.isArray(options.alwaysArray) ? options.alwaysArray.indexOf(key) !== -1 : options.alwaysArray) &&
        ...
        if (options.addParent /*&& key.indexOf('declaration') === -1*/
        && key.indexOf('attributes') === -1 && key.indexOf('instruction') === -1) {
          if (obj[key] instanceof Array) {
            ...
            if (options.compact) {
              if (typeof obj[key] === 'object') obj[key]._parent = obj;
            } else {
              obj[key].parent = obj;
            }
          }
  ...
  for (i = 0; i < cases.length; ++i) {
    ...
    if (direction === 'xml2js') {
      ...
      if (!('spaces' in options) || options.spaces === 0 || typeof options.spaces === 'boolean') {
        tests[i].xml = tests[i].xml.replace(/>\n\v*/gm, '>');
      }
      if ('spaces' in options && options.spaces !== 0 && typeof options.spaces === 'number') {
        var whitespaces = ''
        for (let i = 0; i < options.spaces; i++) {
          whitespaces += ' ';
        }
        tests[i].xml = tests[i].xml.replace(/\v/g, whitespaces);
      }
      if ('spaces' in options && typeof options.spaces === 'string') {
        tests[i].xml = tests[i].xml.replace(/\v/g, options.spaces);
      }
      if (options.ignoreText) {
        tests[i].xml = tests[i].xml.replace(/>([\s\S]*?)</gm, '><');
      }
      if (options.ignoreComment) {
        tests[i].xml = tests[i].xml.replace(/<!--.*?-->/gm, '');
      }
      if (options.ignoreCdata) {
        tests[i].xml = tests[i].xml.replace(/<!\[CDATA\[.*?\]\]>/gm, '');
      }
      if (options.ignoreDoctype) {
        tests[i].xml = tests[i].xml.replace(/<!DOCTYPE[\s\S]*>/gm, '');
      }
      if (options.ignoreDeclaration) {
        tests[i].xml = tests[i].xml.replace(/<\?xml[\s\S]*\?>/gm, '');
      }
      if (options.ignoreInstruction) {
        tests[i].xml = tests[i].xml.replace(/<\?(?!xml)[\s\S]*\?>/gm, '');
      }
      if (options.fullTagEmptyElement) {
        tests[i].xml = tests[i].xml.replace('<a/>', '<a></a>')
          .replace('<b/>', '<b></b>')
          .replace('<c/>', '<c></c>')
          .replace('/>', '></a>');
      }

interface DiyCacheCountPage_Params {
    ratio?: number;
...
function generateId(): string {
    return "DiyCacheCountPage_" + ++__generate__Id;
}
/*
...
class DiyCacheCountPage extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        ...
        this.__currentCacheCount = new ObservedPropertySimple('3', this, "currentCacheCount");
    ...
    updateWithValueParams(params: DiyCacheCountPage_Params) {
        ...
        if (params.currentCacheCount !== undefined) {
            this.currentCacheCount = params.currentCacheCount;
        }
    ...
    aboutToBeDeleted() {
        ...
        this.__currentCacheCount.aboutToBeDeleted();
    ...
    private __currentCacheCount: ObservedPropertySimple<string>;
    get currentCacheCount() {
        return this.__currentCacheCount.get();
    }
    set currentCacheCount(newValue: string) {
        this.__currentCacheCount.set(newValue);
    }
    ...
    async initProxy() {
        ...
        let server: HttpProxyCacheServer = new HttpProxyCacheServerBuilder(getContext())
            .maxCacheFilesCount(Number.parseInt(this.currentCacheCount))
            .build();
        ...
        this.fdPath = newUrl;
        // 由于createAVPlayer方法里面已经设置了url 这里必须重置一下才会生效 直接设置url不生效
        if (this.avPlayer && this.getState() == 'idle') {
    ...
    render() {
        ...
        Row.create();
        Row.alignItems(VerticalAlign.Center);
        Text.create('输入最大缓存个数：');
        Text.fontSize(15);
        Text.width('15%');
        Text.textAlign(TextAlign.Center);
        Text.pop();
        TextInput.create({
            text: this.currentCacheCount,
            placeholder: '请输入最大缓存文件个数，超过此值，将会自动删除多余的缓存文件',
        ...
        Image.onClick(() => {
            if (!this.isSetFinish) {
                promptAction.showToast({
                    message: "请先设置最大缓存大小，然后点击键盘的下一步按钮"
                });
                return;
            }
            this.preVideo();
...
loadDocument(new DiyCacheCountPage("1", undefined, {}));

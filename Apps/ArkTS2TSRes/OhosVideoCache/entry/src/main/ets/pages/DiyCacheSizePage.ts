interface DiyCacheSizePage_Params {
    ratio?: number;
    ...
    server?: HttpProxyCacheServer | null;
}
let __generate__Id: number = 0;
function generateId(): string {
    return "DiyCacheSizePage_" + ++__generate__Id;
}
/*
...
class DiyCacheSizePage extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        ...
        this.__currentCacheSize = new ObservedPropertySimple('512', this, "currentCacheSize");
        this.__isSetFinish = new ObservedPropertySimple(false, this, "isSetFinish");
    ...
    updateWithValueParams(params: DiyCacheSizePage_Params) {
        ...
        if (params.currentCacheSize !== undefined) {
            this.currentCacheSize = params.currentCacheSize;
        }
        if (params.isSetFinish !== undefined) {
            this.isSetFinish = params.isSetFinish;
        }
    ...
    aboutToBeDeleted() {
        ...
        this.__currentCacheSize.aboutToBeDeleted();
        this.__isSetFinish.aboutToBeDeleted();
    ...
    private __currentCacheSize: ObservedPropertySimple<string>;
    get currentCacheSize() {
        return this.__currentCacheSize.get();
    }
    set currentCacheSize(newValue: string) {
        this.__currentCacheSize.set(newValue);
    }
    private __isSetFinish: ObservedPropertySimple<boolean>; // 设置视频缓存的最大缓存容量 仅允许初始化的时候设置一次 设置之后输入框无法使用
    get isSetFinish() {
        return this.__isSetFinish.get();
    }
    set isSetFinish(newValue: boolean) {
        this.__isSetFinish.set(newValue);
    }
    ...
    private server: HttpProxyCacheServer | null;
    // 界面初始化函数
    async aboutToAppear() {
    ...
    async initProxy() {
        ...
        this.server = new HttpProxyCacheServerBuilder(getContext())
            .maxCacheSize(Number.parseInt(this.currentCacheSize) * 1024 * 1024)
            .build();
        this.server?.registerCacheListener(new MyCacheListener(), this.videoName);
        let proxyUrl: string | undefined = await this.server?.getProxyUrl(this.videoName);
    ...
    async initAVPlayer(): Promise<void> {
        ...
        if (!this.fdPath || this.fdPath.length < 1) {
    ...
    async nextVideo(): Promise<void> {
        ...
        this.server?.registerCacheListener(new MyCacheListener(), this.fdPath);
        let proxyUrl: string | undefined = await this.server?.getProxyUrl(this.fdPath);
    ...
    render() {
        ...
        TextInput.create({
            ...
            placeholder: '请输入最大缓存容量，超过此值，将会自动删除多余的缓存文件',
            controller: this.controller
        });
        TextInput.enterKeyType(EnterKeyType.Next);
        TextInput.enabled(!this.isSetFinish);
        ...
        TextInput.height(45);
        TextInput.inputFilter('[0-9]', (value: string) => {
        });
        ...
        Image.onClick(() => {
            if (!this.isSetFinish) {
                promptAction.showToast({
                    message: "请先设置最大缓存容量，然后点击键盘的下一步按钮"
                });
                return;
            }
            this.preVideo();
...
loadDocument(new DiyCacheSizePage("1", undefined, {}));

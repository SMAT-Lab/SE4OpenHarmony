interface SkipAttributesPage_Params {
    message0?: string;
    message1?: string;
    message2?: string;
    message3?: string;
...
function generateId(): string {
    return "SkipAttributesPage_" + ++__generate__Id;
}
/*
...
import { skipp, skipschema } from './IntereceTest';
class SkipAttributesPage extends View {
...
let result = v.validate(p, schema,{skipAttributes:["minimum"]});`, this, "message0");
    ...
    updateWithValueParams(params: SkipAttributesPage_Params) {
    ...
    validate() {
        const ctx = this;
        ctx.message2 = `期待结果：跳过关键字${true} ${'\r\n'}
        不跳过关键字${false}`;
        try {
            let str = JSON.stringify(new Validator().validate(skipp, skipschema, { skipAttributes: ["minimum"] }));
            console.log(`jsonschema ------> 测试结果是：${str}`);
            ctx.message1 = `验证对象： ${str}`;
            ctx.message3 = `实际结果：跳过关键字${new Validator().validate(skipp, skipschema, { skipAttributes: ["minimum"] }).valid} ${'\r\n'}
        不跳过关键字${new Validator().validate(skipp, skipschema).valid}`;
        }
        catch (err) {
...
loadDocument(new SkipAttributesPage("1", undefined, {}));

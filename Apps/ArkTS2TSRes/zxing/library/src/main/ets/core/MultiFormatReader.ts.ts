...
export default class MultiFormatReader implements Reader {
    ...
    public setHints(hints?: Map<DecodeHintType, any> | null): void {
        ...
        if (formats !== null && formats !== undefined) {
            const addOneDReader: boolean = formats.some(f => f === BarcodeFormat.UPC_A ||
            ...
            if (this.includes(formats, BarcodeFormat.QR_CODE)) {
                readers.push(new QRCodeReader());
            }
            if (this.includes(formats, BarcodeFormat.DATA_MATRIX)) {
                readers.push(new DataMatrixReader());
            }
            if (this.includes(formats, BarcodeFormat.AZTEC)) {
                readers.push(new AztecReader());
            }
            if (this.includes(formats, BarcodeFormat.PDF_417)) {
                readers.push(new PDF417Reader());
            }
            if (this.includes(formats, BarcodeFormat.MAXICODE)) {

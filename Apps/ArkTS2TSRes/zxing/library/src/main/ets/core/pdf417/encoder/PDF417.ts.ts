...
import { PDF417ErrorCorrection } from './PDF417ErrorCorrection';
import { PDF417HighLevelEncoder } from './PDF417HighLevelEncoder';
...
export class PDF417 {
     ...
     */
    private static START_PATTERN: int = 0x1fea8;
    /**
     * The stop pattern (18 bits)
     */
    private static STOP_PATTERN: int = 0x3fa29;
    /**
     * The codeword table from the Annex A of ISO/IEC 15438:2001(E).
     */
    private static CODEWORD_TABLE: Int32Array[] = [
        Int32Array.from([0x1d5c0, 0x1eaf0, 0x1f57c, 0x1d4e0, 0x1ea78, 0x1f53e,
        ...
        Int32Array.from([0x1f560, 0x1fab8, 0x1ea40, 0x1f530, 0x1fa9c, 0x1ea20,
        ...
        Int32Array.from([0x1abe0, 0x1d5f8, 0x153c0, 0x1a9f0, 0x1d4fc, 0x151e0,
    ...
    private static PREFERRED_RATIO: float = 3.0;
    private static DEFAULT_MODULE_WIDTH: float = 0.357; // 1px in mm
    private static HEIGHT: float = 2.0; // mm
    ...
    constructor(compact: boolean) {
        ...
        this.encoding = null; // Use default
        this.minCols = 2;
        this.maxCols = 30;
        this.maxRows = 30;
        this.minRows = 2;
    }
    public getBarcodeMatrix(): BarcodeMatrix {
     ...
     */
    private static calculateNumberOfRows(m: int, k: int, c: int): int {
        let r: int = Math.floor(((m + 1 + k) / c) + 1);
        if (c * r >= (m + 1 + k + c)) {
            r--;
        }
        return r;
     ...
     */
    private static getNumberOfPadCodewords(m: int, k: int, c: int, r: int): int {
        let n: int = c * r - k;
        return n > m + 1 ? n - m - 1 : 0;
    }
    private static encodeChar(pattern: int, len: int, logic: BarcodeRow) {
        let map: int = 1 << len - 1;
        let last: boolean = (pattern & map) !== 0; // Initialize to inverse of first bit
        let width: int = 0;
        for (let i: int = 0; i < len; i++) {
            let black: boolean = (pattern & map) !== 0;
            if (last === black) {
                width++;
            }
            else {
    ...
    private encodeLowLevel(fullCodewords: string, c: int, r: int, errorCorrectionLevel: int, logic: BarcodeMatrix) {
        let idx: int = 0;
        for (let y: int = 0; y < r; y++) {
            let cluster: int = y % 3;
            logic.startRow();
            PDF417.encodeChar(PDF417.START_PATTERN, 17, logic.getCurrentRow());
            let left: int;
            let right: int;
            if (cluster === 0) {
                left = (30 * Math.floor(y / 3)) + Math.floor((r - 1) / 3);
                right = (30 * Math.floor(y / 3)) + (c - 1);
            }
            else if (cluster === 1) {
                left = (30 * Math.floor(y / 3)) + (errorCorrectionLevel * 3) + ((r - 1) % 3);
                right = (30 * Math.floor(y / 3)) + Math.floor((r - 1) / 3);
            }
            else {
                left = (30 * Math.floor(y / 3)) + (c - 1);
                right = (30 * Math.floor(y / 3)) + (errorCorrectionLevel * 3) + ((r - 1) % 3);
            }
            let pattern: int = PDF417.CODEWORD_TABLE[cluster][left];
            PDF417.encodeChar(pattern, 17, logic.getCurrentRow());
            for (let x: int = 0; x < c; x++) {
                pattern = PDF417.CODEWORD_TABLE[cluster][StringUtils.getCharCode(fullCodewords.charAt(idx))];
                PDF417.encodeChar(pattern, 17, logic.getCurrentRow());
                idx++;
            }
            if (this.compact) {
                PDF417.encodeChar(PDF417.STOP_PATTERN, 1, logic.getCurrentRow()); // encodes stop line for compact pdf417
            }
            else {
                pattern = PDF417.CODEWORD_TABLE[cluster][right];
                PDF417.encodeChar(pattern, 17, logic.getCurrentRow());
                PDF417.encodeChar(PDF417.STOP_PATTERN, 18, logic.getCurrentRow());
            }
        }
    }
    /**
     ...
     */
    public generateBarcodeLogic(msg: string, errorCorrectionLevel: int): void {
        // 1. step: High-level encoding
        let errorCorrectionCodeWords: int = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(errorCorrectionLevel);
        let highLevel: string = PDF417HighLevelEncoder.encodeHighLevel(msg, this.compaction, this.encoding);
        let sourceCodeWords: int = highLevel.length;
        let dimension: Int32Array = this.determineDimensions(sourceCodeWords, errorCorrectionCodeWords);
        let cols: int = dimension[0];
        let rows: int = dimension[1];
        let pad: int = PDF417.getNumberOfPadCodewords(sourceCodeWords, errorCorrectionCodeWords, cols, rows);
        ...
        let n: int = sourceCodeWords + pad + 1;
        let sb: StringBuilder = new StringBuilder();
        sb.append(String.fromCharCode(n));
        sb.append(highLevel);
        for (let i: int = 0; i < pad; i++) {
            sb.append(String.fromCharCode(900)); // PAD characters
        }
        let dataCodewords: string = sb.toString();
        // 3. step: Error correction
        let ec: string = PDF417ErrorCorrection.generateErrorCorrection(dataCodewords, errorCorrectionLevel);
     ...
     */
    private determineDimensions(sourceCodeWords: int, errorCorrectionCodeWords: int): Int32Array {
        let ratio: float = 0.0;
        let dimension: Int32Array = null;
        for (let cols: int = this.minCols; cols <= this.maxCols; cols++) {
            let rows: int = PDF417.calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, cols);
            ...
            let newRatio: float = ((17 * cols + 69) * PDF417.DEFAULT_MODULE_WIDTH) / (rows * PDF417.HEIGHT);
            // ignore if previous ratio is closer to preferred ratio
            if (dimension != null && Math.abs(newRatio - PDF417.PREFERRED_RATIO) > Math.abs(ratio - PDF417.PREFERRED_RATIO)) {
                continue;
            }
            ratio = newRatio;
            dimension = Int32Array.from([cols, rows]);
        }
        // Handle case when min values were larger than necessary
        if (dimension == null) {
            let rows: int = PDF417.calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, this.minCols);
            if (rows < this.minRows) {
                dimension = Int32Array.from([this.minCols, this.minCols]);
            }
        }
        if (dimension == null) {
     ...
     */
    public setDimensions(maxCols: int, minCols: int, maxRows: int, minRows: int): void {
     ...
     */
    public setCompaction(compaction: Compaction): void {
     ...
     */
    public setEncoding(encoding: Charset): void {

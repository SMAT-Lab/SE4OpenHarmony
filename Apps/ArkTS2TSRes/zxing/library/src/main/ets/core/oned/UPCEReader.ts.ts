...
export default /* final */ class UPCEReader extends UPCEANReader {
    ...
    public decodeMiddle(row: BitArray, startRange: Int32Array, result: string) {
        ...
        let resultStringTemp: StringBuilder = new StringBuilder(result);
        UPCEReader.determineNumSysAndCheckDigit(resultStringTemp, lgPatternFound);
        let resultString = resultStringTemp.toString();
     ...
     */
    // @Override
    decodeEnd(row: BitArray, endStart: int): Int32Array {
        ...
        for (let numSys: int = 0; numSys <= 1; numSys++) {
            for (let d: int = 0; d < 10; d++) {
                if (lgPatternFound === this.NUMSYS_AND_CHECK_DIGIT_PATTERNS[numSys][d]) {
                    resultString.insert(0, /*(char)*/ StringUtils.getCharAt(StringUtils.getCharCode('0') + numSys));
                    resultString.append(/*(char)*/ StringUtils.getCharAt((StringUtils.getCharCode('0') + d)));
                    return;
                }
            }
        }
    ...
    public static convertUPCEtoUPCA(upce: string): string {
        ...
        switch (lastChar) {
            case StringUtils.getCharCode('0'):
            case StringUtils.getCharCode('1'):
            case StringUtils.getCharCode('2'):
                result.appendChars(upceChars, 0, 2);
                result.append(lastChar);
                result.append('0000');
                result.appendChars(upceChars, 2, 3);
                break;
            case StringUtils.getCharCode('3'):
                result.appendChars(upceChars, 0, 3);
                result.append('00000');
                result.appendChars(upceChars, 3, 2);
                break;
            case StringUtils.getCharCode('4'):

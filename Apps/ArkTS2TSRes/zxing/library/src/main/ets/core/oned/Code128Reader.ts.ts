...
export default class Code128Reader extends OneDReader {
        ...
        for (let i = rowOffset; i < width; i++) {
            if (row.get(i) !== isWhite) {
                ...
                if (counterPosition === (patternLength - 1)) {
                    ...
                    if (bestMatch >= 0 &&
                        row.isRange(Math.max(0, patternStart - Math.trunc((i - patternStart) / 2)), patternStart, false)) {
    ...
    public decodeRow(rowNumber: number, row: BitArray, hints?: Map<DecodeHintType, any>): Result {
        ...
        if (!row.isRange(nextStart, Math.min(row.getSize(), nextStart + Math.trunc((nextStart - lastStart) / 2)), false)) {
            throw new NotFoundException();
        }

...
export class PDF417ErrorCorrection {
     ...
     */
    private static EC_COEFFICIENTS: Int32Array[] = [
        Int32Array.from([27, 917]),
        Int32Array.from([522, 568, 723, 809]),
        Int32Array.from([237, 308, 436, 284, 646, 653, 428, 379]),
        Int32Array.from([274, 562, 232, 755, 599, 524, 801, 132, 295, 116, 442, 428, 295,
            42, 176, 65]),
        Int32Array.from([361, 575, 922, 525, 176, 586, 640, 321, 536, 742, 677, 742, 687,
            284, 193, 517, 273, 494, 263, 147, 593, 800, 571, 320, 803,
            133, 231, 390, 685, 330, 63, 410]),
        Int32Array.from([539, 422, 6, 93, 862, 771, 453, 106, 610, 287, 107, 505, 733,
            ...
            623, 264, 543]),
        Int32Array.from([521, 310, 864, 547, 858, 580, 296, 379, 53, 779, 897, 444, 400,
        ...
        Int32Array.from([524, 894, 75, 766, 882, 857, 74, 204, 82, 586, 708, 250, 905,
            ...
            55, 497, 10]),
        Int32Array.from([352, 77, 373, 504, 35, 599, 428, 207, 409, 574, 118, 498, 285,
     ...
     */
    static getErrorCorrectionCodewordCount(errorCorrectionLevel: int): int {
     ...
     */
    static getRecommendedMinimumErrorCorrectionLevel(n: int): int {
        if (n <= 0) {
     ...
     */
    static generateErrorCorrection(dataCodewords: string, errorCorrectionLevel: int): string {
        let k: int = this.getErrorCorrectionCodewordCount(errorCorrectionLevel);
        let e: Int32Array = new Int32Array(k);
        let sld: int = dataCodewords.length;
        for (let i: int = 0; i < sld; i++) {
            let t1: int = (StringUtils.getCharCode(dataCodewords.charAt(i)) + e[e.length - 1]) % 929;
            let t2: int;
            let t3: int;
            for (let j: int = k - 1; j >= 1; j--) {
                t2 = (t1 * this.EC_COEFFICIENTS[errorCorrectionLevel][j]) % 929;
                t3 = 929 - t2;
                e[j] = ((e[j - 1] + t3) % 929);
            }
            t2 = (t1 * this.EC_COEFFICIENTS[errorCorrectionLevel][0]) % 929;
            t3 = 929 - t2;
            e[0] = (t3 % 929);
        }
        let sb: StringBuilder = new StringBuilder();
        for (let j: int = k - 1; j >= 0; j--) {
            if (e[j] !== 0) {
                e[j] = (929 - e[j]);
            }
            sb.append(e[j]);

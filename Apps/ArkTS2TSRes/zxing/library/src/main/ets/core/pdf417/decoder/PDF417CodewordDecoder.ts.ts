...
export default /*final*/ class PDF417CodewordDecoder {
    ...
    private static /*final float[][]*/ RATIOS_TABLE: number[][] = new Array(PDF417Common.SYMBOL_TABLE.length).map(x => x = new Array(PDF417Common.BARS_IN_MODULE));
    /* @note
    ...
    */
    ...
    private static sampleBitCounts(moduleBitCount: Int32Array): Int32Array {
        ...
        for ( /*int*/let i = 0; i < PDF417Common.MODULES_IN_CODEWORD; i++) {
            let sampleIndex: float = bitCountSum / (2 * PDF417Common.MODULES_IN_CODEWORD) +

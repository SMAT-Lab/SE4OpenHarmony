    ...
    public decodeRow(rowNumber: number, row: BitArray, hints?: Map<DecodeHintType, any>): Result {
        for (let reader of this.readers) {
            try {
                ...
                const ean13MayBeUPCA: boolean = result.getBarcodeFormat() === BarcodeFormat.EAN_13 &&
                    result.getText().charAt(0) === '0';
                // @SuppressWarnings("unchecked")
                const possibleFormats: Collection<BarcodeFormat> = hints == null ? null : hints.get(DecodeHintType.POSSIBLE_FORMATS) as Collection<BarcodeFormat>;
                const canReturnUPCA: boolean = possibleFormats == null || this.includes(possibleFormats, BarcodeFormat.UPC_A);
                if (ean13MayBeUPCA && canReturnUPCA) {
                    const rawBytes = result.getRawBytes();
                    // Transfer the metadata across
                    const resultUPCA: Result = new Result(result.getText().substring(1), rawBytes, rawBytes === null ? 0 : rawBytes.length, result.getResultPoints(), BarcodeFormat.UPC_A);

...
export default /*public*/ /*final*/ class Detector {
    ...
    private static /*final*/ MAX_PIXEL_DRIFT: number = 3;
    private static /*final*/ MAX_PATTERN_DRIFT: number = 5;
    // if we set the value too low, then we don't detect the correct height of the bar if the start patterns are damaged.
    // if we set the value too high, then we might detect the start pattern from a neighbor barcode.
    private static /*final*/ SKIPPED_ROW_COUNT_MAX: number = 25;
    // A PDF471 barcode should have at least 3 rows, with each row being >= 3 times the module width. Therefore it should be at least
    // 9 pixels tall. To be conservative, we use about half the size to ensure we don't miss it.
    private static /*final*/ ROW_STEP: number = 5;
    private static /*final*/ BARCODE_MIN_HEIGHT: number = 10;
    /**
     ...
     */
    private static findVertices(matrix: BitMatrix, startRow: number, startColumn: number): ResultPoint[] {
    ...
    private static findRowsWithPattern(matrix: BitMatrix, height: number, width: number, startRow: number, startColumn: number, pattern: Int32Array): ResultPoint[] {
     ...
     */
    private static findGuardPattern(matrix: BitMatrix, column: number, row: number, width: number, whiteFirst: boolean, pattern: Int32Array, counters: Int32Array): Int32Array {

...
export default /*public final*/ class AztecWriter implements Writer {
    ...
    public encodeWithHints(contents: string, format: BarcodeFormat, width: int, height: int, hints: Map<EncodeHintType, any>): BitMatrix {
        ...
        if (hints != null) {
            ...
            if (hints.has(EncodeHintType.ERROR_CORRECTION)) {
                eccPercent = Integer.parse(hints.get(EncodeHintType.ERROR_CORRECTION).toString());
            }
            if (hints.has(EncodeHintType.AZTEC_LAYERS)) {
                layers = Integer.parse(hints.get(EncodeHintType.AZTEC_LAYERS).toString());
            }
        }
        return AztecWriter.encodeLayers(contents, format, width, height, charset, eccPercent, layers);
    }
    private static encodeLayers(contents: string, format: BarcodeFormat, width: int, height: int, charset: Charset, eccPercent: int, layers: int): BitMatrix {
    ...
    private static renderResult(code: AztecCode, width: int, height: int): BitMatrix {
        ...
        let multiple: int = Math.floor(Math.min(outputWidth / inputWidth, outputHeight / inputHeight));
        let leftPadding: int = Math.floor((outputWidth - (inputWidth * multiple)) / 2);
        let topPadding: int = Math.floor((outputHeight - (inputHeight * multiple)) / 2);

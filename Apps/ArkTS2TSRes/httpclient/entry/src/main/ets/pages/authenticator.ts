interface authenticator_Params {
    status?: string;
    content?: string;
    count?: number;
    client?: HttpClient;
...
function generateId(): string {
    return "authenticator_" + ++__generate__Id;
}
/*
 ...
 */
import { HttpClient, NetAuthenticator, Request, Response } from '@ohos/httpclient';
import prompt from '@system.prompt';
import Log from '../model/log';
import hilog from '@ohos.hilog';
import { BusinessError } from '@ohos/httpclient/src/main/ets/http';
class authenticator extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        ...
        this.count = 0;
        this.client = new HttpClient.Builder().setConnectTimeout(10000)
            .authenticator(new NetAuthenticator('jesse', 'password1'))
            .build();
        this.updateWithValueParams(params);
    }
    updateWithValueParams(params: authenticator_Params) {
...
loadDocument(new authenticator("1", undefined, {}));

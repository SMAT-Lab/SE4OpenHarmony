interface unidirectionalCustomCertificate_Params {
    url?: string;
    wanAndroidUrl?: string;
    result?: string;
    ca?;
    wanAndroidRootCA?;
    wanAndroidRSACA?;
...
function generateId(): string {
    return "unidirectionalCustomCertificate_" + ++__generate__Id;
}
/*
 ...
 * 自定义证书配置单向向证书验证
 */
const TAG: string = "customCertificate_Unidirectional";
class unidirectionalCustomCertificate extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        super(compilerAssignedUniqueChildId, parent, localStorage);
        this.__url = new ObservedPropertySimple("https://1.94.37.200:8080/user/getUserByUuid?userUuid=1", this, "url");
        this.__wanAndroidUrl = new ObservedPropertySimple("https://www.wanandroid.com/hotkey/json", this, "wanAndroidUrl");
        this.__result = new ObservedPropertySimple("响应结果", this, "result");
        this.ca = "noPassword/ca.crt";
        this.wanAndroidRootCA = "wanandroidRoot.crt";
        this.wanAndroidRSACA = "wanandroidRSA.crt";
        this.updateWithValueParams(params);
    }
    updateWithValueParams(params: unidirectionalCustomCertificate_Params) {
        if (params.url !== undefined) {
            this.url = params.url;
        }
        if (params.wanAndroidUrl !== undefined) {
            this.wanAndroidUrl = params.wanAndroidUrl;
        }
        ...
        if (params.wanAndroidRootCA !== undefined) {
            this.wanAndroidRootCA = params.wanAndroidRootCA;
        }
        if (params.wanAndroidRSACA !== undefined) {
            this.wanAndroidRSACA = params.wanAndroidRSACA;
        }
    }
    aboutToBeDeleted() {
        this.__url.aboutToBeDeleted();
        this.__wanAndroidUrl.aboutToBeDeleted();
    ...
    private __wanAndroidUrl: ObservedPropertySimple<string>;
    get wanAndroidUrl() {
        return this.__wanAndroidUrl.get();
    }
    set wanAndroidUrl(newValue: string) {
        this.__wanAndroidUrl.set(newValue);
    }
    ...
    private ca;
    private wanAndroidRootCA;
    private wanAndroidRSACA;
    render() {
        Column.create();
        Column.width('100%');
        Column.height('100%');
        ...
        Button.pop();
        TextInput.create({ text: this.wanAndroidUrl, placeholder: '请输入URL' });
        TextInput.placeholderColor("#ffffff");
        TextInput.caretColor(Color.Blue);
        TextInput.height("150px");
        TextInput.fontSize("18fp");
        TextInput.onChange((value: string) => {
            this.wanAndroidUrl = value;
        });
        Button.createWithLabel('GET请求1：校验通过请求');
        Button.width('80%');
        Button.height('80px');
        ...
        Button.onClick(async (event: ClickEvent) => {
            ...
            Logger.info(TAG, 'HttpClient end');
            let context: Context = getContext();
            let rootCAData: string = await new GetCAUtils().getCA(this.wanAndroidRootCA, context);
            let rsaCAData: string = await new GetCAUtils().getCA(this.wanAndroidRSACA, context);
            Logger.info(TAG, 'request begin');
            Logger.info(TAG, 'rootCAData:', JSON.stringify(rootCAData));
            Logger.info(TAG, 'rsaCAData:', JSON.stringify(rsaCAData));
            let request: Request = new Request.Builder()
                .url(this.wanAndroidUrl)
                .method('GET')
                .ca([rootCAData, rsaCAData])
                .build();
            Logger.info(TAG, 'request end');
            client.newCall(request)
                .checkCertificate(new SslCertificateManagerSuccess())
...
loadDocument(new unidirectionalCustomCertificate("1", undefined, {}));

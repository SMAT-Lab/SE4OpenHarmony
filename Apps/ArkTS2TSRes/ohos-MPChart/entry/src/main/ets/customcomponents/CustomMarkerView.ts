...
function generateId(): string {
    return "CustomMarkerView_" + ++__generate__Id;
}
/*
 ...
 */
import { BarEntry, ChartData, ChartModel, EntryOhos, Highlight, IDataSet, IMarker, MPPointF } from '@ohos/mpchart';
export default class MarkerView implements IMarker {
    ...
    draw(c: CanvasRenderingContext2D, posX: number, posY: number, e: EntryOhos, highlight: Highlight) {
        ...
        c.save();
        let entry = e as BarEntry;
        let yValues: number[] | null = entry.getYVals();
        if (!yValues) {
            return;
        }
        let roundX = Number(entry.getX().toFixed(1));
        let roundY = Number(entry.getY().toFixed(1));
        let highLightY = yValues[highlight.getStackIndex()];
        if (highLightY) {
            roundY = Number(highLightY.toFixed(1));
        }

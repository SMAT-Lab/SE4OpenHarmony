...
function generateId(): string {
    return "addimage.test_" + ++__generate__Id;
}
/*
...
export default function addimageTest() {
    describe('AddimageTest', () => {
        ...
        it('image_rotation_45', 0, () => {
            ...
            let content: Array<number> = getContent(doc, jpg, "JPEG", 300, 200, 280, 210, undefined, undefined, 45);
            expect(content.length).assertEqual(15714);
            let buf: ArrayBuffer = getArrayBuffer(doc, content);
            expect(new Uint8Array(buf) != undefined).assertTrue();
            expect(buf.byteLength).assertEqual(15714);
            const writer = fs.openSync(data + `/rotation-45.pdf`, 0o102);
            fs.writeSync(writer.fd, buf);
            fs.close(writer);
            let startTime = new Date().getTime();
            for (let index = 0; index < BASE_COUNT; index++) {
                getContent(doc, jpg, "JPEG", 300, 200, 280, 210, undefined, undefined, 45);
            }
            endTime(startTime, 'image_rotation_45');
        });
        it("image_rotation_90", 0, () => {
            let gloContext: Context = GlobalContext.getContext().getObject("context") as Context;
            let options: Options = {
                orientation: "p",
                unit: "pt",
            ...
            const doc: jsPDF = new jsPDF(options);
            let content: Array<number> = getContent(doc, jpg, "JPEG", 300, 200, 280, 210, undefined, undefined, 90);
            expect(content.length).assertEqual(15714);
            let buf: ArrayBuffer = getArrayBuffer(doc, content);
            expect(new Uint8Array(buf) != undefined).assertTrue();
            expect(buf.byteLength).assertEqual(15714);
            const writer = fs.openSync(gloContext.filesDir + `/rotation-90.pdf`, 0o102);
            fs.writeSync(writer.fd, buf);
            fs.close(writer);
            let startTime = new Date().getTime();
            for (let index = 0; index < BASE_COUNT; index++) {
                getContent(doc, jpg, "JPEG", 300, 200, 280, 210, undefined, undefined, 90);
            }
            endTime(startTime, 'image_rotation_45');
        });
        it("sHashCode", 0, () => {
            const doc: jsPDF = new jsPDF();
            expect(getSHashCode(doc)).assertEqual(0);
            expect(getSHashCode(doc, "testtest")).assertEqual(-1145835484);
            let startTime = new Date().getTime();
            for (let index = 0; index < BASE_COUNT; index++) {
                getSHashCode(doc);
            }

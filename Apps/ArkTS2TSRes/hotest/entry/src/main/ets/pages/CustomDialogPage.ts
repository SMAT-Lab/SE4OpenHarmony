interface CustomDialogPage_Params {
    textValue?: string;
    inputValue?: string;
...
function generateId(): string {
    return "CustomDialogPage_" + ++__generate__Id;
}
...
class CustomDialogPage extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        super(compilerAssignedUniqueChildId, parent, localStorage);
        this.__textValue = new ObservedPropertySimple('', this, "textValue");
        this.__inputValue = new ObservedPropertySimple('click me'
        //CustomDialogController仅在作为@CustomDialog和@Component struct的成员变量，且在@Component struct内部定义时赋值才有效
        , this, "inputValue");
    ...
    updateWithValueParams(params: CustomDialogPage_Params) {
...
loadDocument(new CustomDialogPage("1", undefined, {}));

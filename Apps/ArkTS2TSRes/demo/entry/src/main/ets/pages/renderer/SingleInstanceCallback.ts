...
class SingleInstanceCallback extends View {
    ...
    setAudioEffectMode() {
        ...
        let _this = this;
        _this.audioRenderer.setAudioEffectMode(this.audioEffectMode, (err) => {
            if (err) {
    ...
    getAudioEffectMode() {
        ...
        let _this = this;
        _this.audioRenderer.getAudioEffectMode((err, effectmode) => {
            if (err) {
    ...
    getRendererInfo() {
        ...
        let _this = this;
        _this.audioRenderer.getRendererInfo((err, capturerInfo) => {
            if (err) {
                _this.returnMsg = `audioRenderer getRendererInfo : Error: ${JSON.stringify(err)}`;
            }
            else {
                _this.returnMsg = `audioRenderer getRendererInfo : Success: ${JSON.stringify(capturerInfo)}`;
            }
        });
    }
    getStreamInfo() {
        ...
        let _this = this;
        _this.audioRenderer.getStreamInfo((err, streamInfo) => {
            if (err) {
    ...
    getAudioTime() {
        ...
        let _this = this;
        _this.audioRenderer.getAudioTime((err, timestamp) => {
            if (err) {
    ...
    getAudioStreamId() {
        ...
        let _this = this;
        _this.audioRenderer.getAudioStreamId((err, streamid) => {
            if (err) {
    ...
    render() {
        ...
        Column.position({ x: '1%' });
        Text.create("【音频渲染-单实例-callback】返回数据：");
        ...
        Select.onSelect((index, value) => {
            ...
            if (value == "CONTENT_TYPE_ULTRASONIC") {
                this.audioRendererOptions.rendererInfo.content = 9;
            }
        });
        Select.pop();
        Row.pop();
        ...
        Select.onSelect((index, value) => {
            ...
            if (value == "STREAM_USAGE_ALARM") {
                this.audioRendererOptions.rendererInfo.usage = 4;
            }
            if (value == "STREAM_USAGE_ACCESSIBILITY") {
                this.audioRendererOptions.rendererInfo.usage = 8;
            }
            if (value == "STREAM_USAGE_SYSTEM") {
                this.audioRendererOptions.rendererInfo.usage = 9;
            }
        });
        Select.pop();
        Row.pop();

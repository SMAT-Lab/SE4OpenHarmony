interface GetSystemVolumeInDb_Params {
...
function generateId(): string {
    return "GetSystemVolumeInDb_" + ++__generate__Id;
}
/*
...
class GetSystemVolumeInDb extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        ...
        this.__selectedVolumeTypeKey = new ObservedPropertySimple("请选择音频", this, "selectedVolumeTypeKey");
        this.__selectedVolumeType = new ObservedPropertySimple(-1, this, "selectedVolumeType");
        this.volumeTypeSelectList = [];
        this.__selectedDeviceTypeKey = new ObservedPropertySimple("请选择设备", this, "selectedDeviceTypeKey");
        this.__selectedDeviceType = new ObservedPropertySimple(2, this, "selectedDeviceType");
        this.deviceTypeSelectList = [];
    ...
    updateWithValueParams(params: GetSystemVolumeInDb_Params) {
        ...
        if (params.selectedDeviceTypeKey !== undefined) {
            this.selectedDeviceTypeKey = params.selectedDeviceTypeKey;
        }
        if (params.selectedDeviceType !== undefined) {
            this.selectedDeviceType = params.selectedDeviceType;
        }
        if (params.deviceTypeSelectList !== undefined) {
            this.deviceTypeSelectList = params.deviceTypeSelectList;
        }
    ...
    aboutToBeDeleted() {
        ...
        this.__selectedDeviceTypeKey.aboutToBeDeleted();
        this.__selectedDeviceType.aboutToBeDeleted();
    ...
    private __selectedDeviceTypeKey: ObservedPropertySimple<string>;
    get selectedDeviceTypeKey() {
        return this.__selectedDeviceTypeKey.get();
    }
    set selectedDeviceTypeKey(newValue: string) {
        this.__selectedDeviceTypeKey.set(newValue);
    }
    private __selectedDeviceType: ObservedPropertySimple<number>; //设备类型
    get selectedDeviceType() {
        return this.__selectedDeviceType.get();
    }
    set selectedDeviceType(newValue: number) {
        this.__selectedDeviceType.set(newValue);
    }
    private deviceTypeSelectList;
    ...
    async aboutToAppear() {
        ...
        for (let key in audio.DeviceType) {
            this.deviceTypeSelectList.push({ value: key });
        }
    ...
    render() {
        ...
        Select.pop();
        Select.create(this.deviceTypeSelectList);
        Select.value(this.selectedDeviceTypeKey);
        Select.onSelect((index, value) => {
            this.selectedDeviceType = audio.DeviceType[value];
            this.selectedDeviceTypeKey = value;
        });
        Select.pop();
        Row.pop();
        If.create();
        if (this.selectedVolumeTypeKey !== "请选择音频") {
            If.branchId(0);
            Divider.create();
        ...
        Button.onClick(() => {
            let _this = this;
            this.audioVolumeGroupManager.getSystemVolumeInDb(this.selectedVolumeType, this.selectedVolume, this.selectedDeviceType, (err, value) => {
                console.info(`VolumeType: ${this.selectedVolumeType}, Volume: ${this.selectedVolume}, DeviceType: ${this.selectedDeviceType}`);
                if (err) {
                    _this.returnMsg = 'getSystemVolumeInDb Callback err:' + JSON.stringify(err);
                    return;
                }
                _this.returnMsg = "getSystemVolumeInDb Callback:SUCCESS 返回值 " + value;
            });
        });
        Text.create("getSystemVolumeInDb Callback");
        Text.fontSize(20);
        ...
        Button.onClick(() => {
            let _this = this;
            this.audioVolumeGroupManager.getSystemVolumeInDb(this.selectedVolumeType, this.selectedVolume, this.selectedDeviceType)
                .then(value => {
                _this.returnMsg = "getSystemVolumeInDb Promise:SUCCESS 返回值 " + value;
            })
                .catch((err) => {
                _this.returnMsg = 'getSystemVolumeInDb Promise err:' + JSON.stringify(err);
            });
        });
        Text.create("getSystemVolumeInDb Promise");
        Text.fontSize(20);
        Text.fontColor(Color.White);
        Text.pop();
        Button.pop();
...
loadDocument(new GetSystemVolumeInDb("1", undefined, {}));

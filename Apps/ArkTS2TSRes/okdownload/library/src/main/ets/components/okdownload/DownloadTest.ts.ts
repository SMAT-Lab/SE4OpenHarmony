...
export class DownloadTest {
     ...
     * {@link #enqueue(DownloadTaskTask[], DownloadListener)} instead, because the performance is
     ...
     * <p>
     * If there are more than one task need to cancel, please using {@link #cancel(DownloadTaskTask[])}
    ...
    //   const builder = new DownloadTest.Builder(anotherUrl, anotherUri)
    ...
    //   isWifiRequired : boolean = DownloadTest.Builder.DEFAULT_IS_WIFI_REQUIRED;
    //
    ...
    //    * {@link StatusUtil.Status#isCompleted(DownloadTaskTask)}, callback completed directly instead
    //    * of start download.
    //    */
    //   passIfAlreadyCompleted: boolean = DownloadTest.Builder.DEFAULT_PASS_IF_ALREADY_COMPLETED;
    //
    //   /**
    ...
    //     if (this.headerMapFields == null) this.headerMapFields = {};
    ...
    //     // return new DownloadTaskTask(this.url, this.filename, this.parentPath, this.priority, this.headerMapFields, this.passIfAlreadyCompleted, this.isWifiRequired, this.isFilenameFromResponse)
    //   }
    // };
    /**
    ...
    /**
     * Correspond to [DownloadTaskTask.execute].
   ...
   */
    ...
    /**
     * Correspond to [DownloadTaskTask.enqueue].
     * This method will create a [DownloadListener] instance internally.
     */
...
export class MockTaskForCompare {
    id: number;
    url: string;
    filename: string;
    public constructor(id: number, task?: DownloadTest) {
        this.id = id;
        if (task) {

...
class Sample extends SampleListener {
    private that: any;
    ...
    public taskBegin(task: DownloadTask): void {
        Log.showDebug(TAG, "==== begin ==== " + task.getId());
        this.that.fillPbInfo(task, "start");
        task.addTag(this.that.CURRENT_PROGRESS, 0);
    }
    ;
    public retry(task: DownloadTask, cause: ResumeFailedCause): void {
        this.that.fillPbInfo(task, "retry");
    }
    ;
    public connected(task: DownloadTask, blockCount: number, totalLength: number): void {
        this.that.fillPbInfo(task, "connected");
    }
    ;
    public progress(task: DownloadTask, currentOffset: number, totalLength: number): void {
        //Log.showDebug(TAG, "=== sample progress = " + task.getId())
        this.that.calcSpeed(task, currentOffset);
        this.that.fillPbInfo(task, "progress");
        task.addTag(this.that.CURRENT_PROGRESS, currentOffset);
        let taskView: any = this.that.findValidTaskViewsWithId(task.getId());
        taskView.progress = Math.floor(currentOffset / totalLength * 100);
    }
    ;
    public taskCompleted(task: DownloadTask, cause: EndCause, realCause: Error): void {
        this.that.currentCount += 1;
        this.that.totalTask = "Total Progress: " + this.that.currentCount + "/" + this.that.totalCount + "(" + this.that.speedCalculator.speed() + ")";
        this.that.fillPbInfo(task, EndCause[cause]);
        Log.showDebug(TAG, "==== sample onComplete id == " + task.getId() + "; cause = " + EndCause[cause] + "; currentCount = " + this.that.currentCount);
        this.that.releaseTaskViewsWithId(task.getId());
    }
    ;
}
class DownloadContextListenerSample implements DownloadContextListener {
    ...
    public taskEnd(context: DownloadContext, task: DownloadTask, cause: EndCause, realCause: Error, remainCount: number): void {
        Log.showInfo(TAG, "====== taskEnd ======= " + remainCount);
    }
    ;
    public queueEnd(context: DownloadContext): void {
        Log.showInfo(TAG, "====== queueEnd =======");
        this.that.status = "START";
        this.that.isEnable = true;
    }
    ;
}
class Bunch extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        super(compilerAssignedUniqueChildId, parent, localStorage);
        this.isStarted = false;
        this.__status = new ObservedPropertySimple("START", this, "status");
        this.__isSerial = new ObservedPropertySimple(0, this, "isSerial");
        this.__isEnable = new ObservedPropertySimple(true, this, "isEnable");
        this.__totalTask = new ObservedPropertySimple("Total Progress:-/-", this, "totalTask");
        this.__totalCount = new ObservedPropertySimple(0, this, "totalCount");
        this.__currentCount = new ObservedPropertySimple(0, this, "currentCount");
        this.speedCalculator = null;
        this.downloadContext = null;
        this.INDEX_TAG = 1;
        this.CURRENT_PROGRESS = 2;
        this.__views = new ObservedPropertyObject([{ text: "Task:-", progress: 0, id: 0 },
            { text: "Task:-", progress: 0, id: 0 },
            { text: "Task:-", progress: 0, id: 0 },
            { text: "Task:-", progress: 0, id: 0 },
            { text: "Task:-", progress: 0, id: 0 }], this, "views");
        this.urls = [
            // 随机小资源一般不超过10
    ...
    updateWithValueParams(params: Bunch_Params) {
        ...
        if (params.currentCount !== undefined) {
            this.currentCount = params.currentCount;
        }
        if (params.speedCalculator !== undefined) {
            this.speedCalculator = params.speedCalculator;
        }
        if (params.downloadContext !== undefined) {
            this.downloadContext = params.downloadContext;
        }
        if (params.INDEX_TAG !== undefined) {
            this.INDEX_TAG = params.INDEX_TAG;
        }
        if (params.CURRENT_PROGRESS !== undefined) {
            this.CURRENT_PROGRESS = params.CURRENT_PROGRESS;
        }
    ...
    aboutToBeDeleted() {
        ...
        this.__currentCount.aboutToBeDeleted();
        this.__views.aboutToBeDeleted();
    ...
    private __totalTask: ObservedPropertySimple<string>;
    get totalTask() {
        return this.__totalTask.get();
    }
    set totalTask(newValue: string) {
    ...
    private __currentCount: ObservedPropertySimple<number>;
    get currentCount() {
        return this.__currentCount.get();
    }
    set currentCount(newValue: number) {
        this.__currentCount.set(newValue);
    }
    private speedCalculator: SpeedCalculator | null;
    private downloadContext: DownloadContext | null;
    private readonly INDEX_TAG;
    private readonly CURRENT_PROGRESS;
    private __views: ObservedPropertyObject<ViewEntity[]>;
    get views() {
        return this.__views.get();
    }
    set views(newValue: ViewEntity[]) {
        this.__views.set(newValue);
    }
    render() {
        ...
        Column.width('50%');
        Column.padding(10);
        Button.createWithLabel(this.status, { type: ButtonType.Normal, stateEffect: true });
        Button.width('180vp');
        ...
        Button.height('50vp');
        Button.fontColor(0x868686);
        Button.enabled(this.isEnable);
        Button.backgroundColor(this.isEnable ? 0xFFFFFF : 0x9E9E9E);
        Button.border({ width: 1, color: 0xE6E6E6, style: BorderStyle.Solid });
        Button.onClick(() => {
        ...
        Text.create(this.views[0].text);
        Text.width("100%");
        Text.fontColor(0x000000);
        Text.fontSize(26);
        Text.pop();
        Progress.create({
            value: this.views[0].progress,
            total: 100,
            style: ProgressStyle.Linear
        });
        Progress.width('100%');
        Progress.color(0xFF4081);
        Text.create(this.views[1].text);
        Text.width("100%");
        Text.fontColor(0x000000);
        Text.fontSize(26);
        Text.pop();
        Progress.create({
            value: this.views[1].progress,
            total: 100,
            style: ProgressStyle.Linear
        });
        Progress.width('100%');
        Progress.color(0xFF4081);
        Text.create(this.views[2].text);
        Text.width("100%");
        Text.fontColor(0x000000);
        Text.fontSize(26);
        Text.pop();
        Progress.create({
            value: this.views[2].progress,
            total: 100,
            style: ProgressStyle.Linear
        });
        Progress.width('100%');
        Progress.color(0xFF4081);
        Text.create(this.views[3].text);
        Text.width("100%");
        Text.fontColor(0x000000);
        Text.fontSize(26);
        Text.pop();
        Progress.create({
            value: this.views[3].progress,
            total: 100,
            style: ProgressStyle.Linear
        });
        Progress.width('100%');
        Progress.color(0xFF4081);
        Text.create(this.views[4].text);
        Text.width("100%");
        Text.fontColor(0x000000);
        Text.fontSize(26);
        Text.pop();
        Progress.create({
            value: this.views[4].progress,
            total: 100,
            style: ProgressStyle.Linear
        });
        Progress.width('100%');
    ...
    onPageShow() {
        let builder: any = new QueueSet()
            .setParentPath("brunch")
            .commit();
        this.urls.forEach((value, index) => {
            builder.bindUrl(value, "bunch" + index).addTag(this.INDEX_TAG, index + 1);
        });
        this.totalCount = this.urls.length;
        this.currentCount = 0;
        this.downloadContext = builder.setListener(new DownloadContextListenerSample(this)).build();
        this.speedCalculator = new SpeedCalculator();
    }
    private startDownload() {
        if (this.downloadContext != null) {
            this.downloadContext.start(new Sample(this), this.isSerial == 0);
        }
    }
    private calcSpeed(task: DownloadTask, currentOffset: number) {
        let progressValue: number = task.getTag(this.CURRENT_PROGRESS);
        let preOffset: number = 0;
        if (progressValue == null) {
            preOffset = 0;
        }
        else {
            preOffset = progressValue;
        }
        let increase = currentOffset - preOffset;
        if (this.speedCalculator != null) {
            this.speedCalculator.downloading(increase);
            this.totalTask = "Total Progress: " + this.currentCount + "/" + this.totalCount + "(" + this.speedCalculator.speed() + ")";
        }
    }
    private fillPbInfo(task: DownloadTask, info: string) {
        const index: any = task.getTag(this.INDEX_TAG);
        let taskViews = this.findValidTaskViewsWithId(task.getId());
        if (taskViews == null)
            return;
        taskViews.text = "Task: " + index + " [" + info + "]";
    }
    private findValidTaskViewsWithId(taskId: number): ViewEntity {
        if (this.isSerial == 0) {
            this.views[0].id == taskId;
            return this.views[0];
        }
        for (let i = 0; i < this.views.length; i++) {
            if (this.views[i].id == taskId)
                return this.views[i];
        }
        for (let i = 0; i < this.views.length; i++) {
            if (this.views[i].id == 0) {
                this.views[i].id = taskId;
                return this.views[i];
            }
        }
        return new ViewEntity();
    }
    private releaseTaskViewsWithId(taskId: number) {
        for (let i = 0; i < this.views.length; i++) {
            if (this.views[i].id == taskId)
                this.views[i].id = 0;
        }
...
loadDocument(new Bunch("1", undefined, {}));

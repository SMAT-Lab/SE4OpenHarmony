...
export class Client {
            ...
            if (this._state == State.NBDaDp ||
                ...
                if (this._retrans < 9) {
                    ...
                    */
                    ...
                    this._logger.info(Logger.clientEventId, "NB-Rtx0: len=" + sbuf.byteLength +
    ...
    setServerAddress(host: string, port: number) {
        var d = host.split('.');
        if (d.length != 4 || (isNaN(parseInt(d[0])) ||
            isNaN(parseInt(d[1])) ||
            isNaN(parseInt(d[2])) ||
            isNaN(parseInt(d[3])))) {
            this._domain = host;
    ...
    getNatType() {
        ...
        if (nb == Const.Type.I) {
            ...
            if (ef == Const.Type.I)
                return "Full cone";
            if (ef == Const.Type.PD)
                return "Port-only-restricted cone";
            if (ef == Const.Type.AD)

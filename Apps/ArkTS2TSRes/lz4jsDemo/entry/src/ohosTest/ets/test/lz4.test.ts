...
function generateId(): string {
    return "lz4.test_" + ++__generate__Id;
}
/*
...
export default function lz4Test() {
    ...
    describe('lz4Test', () => {
        it('shouldDecompressEmptyLz4ArrayCorrectly', 0, () => {
            let emptyLz4: number[] = [4, 34, 77, 24, 64, 112, 223, 0, 0, 0, 0];
            expect(lz4.decompress(emptyLz4)).assertDeepEquals(byteArray([]));
            //expect(lz4.decompress(emptyLz4)).to.be.deep.equal(byteArray(0));
        });
        it('shouldDecompressEmptyLz4Uint8ArrayCorrectly', 0, () => {
            let emptyLz4: Uint8Array | Array<number> = byteArray([4, 34, 77, 24, 64, 112, 223, 0, 0, 0, 0]);
            expect(lz4.decompress(emptyLz4)).assertDeepEquals(byteArray([]));
            //expect(lz4.decompress(emptyLz4)).to.be.deep.equal(byteArray(0));
        });
        it('shouldDecompressDataCompressedWithLz4c', 1, () => {
            let output: Uint8Array | Array<number> = byteArray([
                0x54, 0x68, 0x65, 0x20, 0x77, 0x68, 0x6f, 0x6c,
                0x65, 0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64, 0x20,
                0x69, 0x73, 0x20, 0x65, 0x6e, 0x64, 0x69, 0x6e,
                0x67, 0x2e, 0x0a
            ]);
            let input: Uint8Array | Array<number> = byteArray([
            ...
            expect(lz4.decompress(input)).assertDeepEquals(output);
            //expect(lz4.decompress(input)).to.be.deep.equal(output);
        });
        it('shouldDecompressDataCompressedWithLz4c2', 2, () => {
            let output: Uint8Array | Array<number> = byteArray([
        ...
        it('shouldDecompressDataContainingContentSize', 3, () => {
            let output: Uint8Array | Array<number> = byteArray([
        ...
        it('shouldFailOnBadHeaderMagic', 4, () => {
            let input: Uint8Array | Array<number> = byteArray([5, 34, 77, 24, 64, 112, 223, 0, 0, 0, 0]);
            let fn = (): Uint8Array | number[] => {
                return lz4.decompress(input);
            };
            expect(fn).assertThrowError("invalid magic number");
        });
        it('shouldFailOnBadBlockSize', 5, () => {
            let input: Uint8Array | Array<number> = byteArray([4, 34, 77, 24, 64, 0, 223, 0, 0, 0, 0]);
            let fn = (): Uint8Array | number[] => {
                return lz4.decompress(input);
            };
            expect(fn).assertThrowError("invalid block size 0");
        });
        it('shouldPassTheLinuxKernelLz4TestVector', 6, () => {
            ...
            let linuxTestIn: Uint8Array | Array<number> = byteArray([
            ...
            let linuxTestOut: Uint8Array | Array<number> = byteArray([
            ...
            let testOut: Uint8Array | Array<number> = byteArray(70);
            lz4.decompressBlock(linuxTestIn, testOut, 0, 45, 0);
            expect(testOut).assertDeepEquals(linuxTestOut);
        });
        it('shouldCompressEmptyArrayCorrectly', 7, () => {
            let emptyLz4: Uint8Array | Array<number> = byteArray([4, 34, 77, 24, 64, 112, 223, 0, 0, 0, 0]);
            expect(lz4.compress([])).assertDeepEquals(emptyLz4);
        });
        it('shouldCompressEmptyUint8ArrayCorrectly', 8, () => {
            let emptyLz4: Uint8Array | Array<number> = byteArray([4, 34, 77, 24, 64, 112, 223, 0, 0, 0, 0]);
            expect(lz4.compress(byteArray([]))).assertDeepEquals(emptyLz4);
        });
        it('shouldOutputPseudoRLE', 9, () => {
            let input: Uint8Array | Array<number> = byteArray([
            ...
            expect(lz4.compress(input)).assertDeepEquals(output);
        });
        it('shouldFindMatches', 10, () => {
            let input: Uint8Array | Array<number> = byteArray([
        ...
        it('shouldUseMaxSize', 11, () => {
            let emptyLz4: Uint8Array | Array<number> = byteArray([4, 34, 77, 24, 64, 112, 223, 0]);
            expect(lz4.compress([], 8)).assertDeepEquals(emptyLz4);
        });
        it('shouldNotCompressUncompressibleData', 12, () => {
            let input: Uint8Array | Array<number> = byteArray([
        ...
        it('shouldBeAbleToOutputOver15literalsAtEnd', 13, () => {
            let input: Uint8Array | Array<number> = byteArray([
                0x4a, 0x6f, 0x69, 0x6e, 0x20, 0x75, 0x73, 0x20,
                0x6e, 0x6f, 0x77, 0x20, 0x61, 0x6e, 0x64, 0x20,
                0x73, 0x68, 0x61, 0x72, 0x65, 0x20, 0x74, 0x68,
            ...
            let testOut: Uint8Array | Array<number> = byteArray(59);
            let sz: number = lz4.compressBlock(input, testOut, 0, input.length, lz4.makeBuffer(1 << 16));
            expect(sz).assertEqual(59);
            expect(testOut).assertDeepEquals(output);
        });
    });
}

...
function generateId(): string {
    return "GridLayoutHelper_" + ++__generate__Id;
}
/*
...
export class GRID_LAYOUT extends View {
    ...
    updateWithValueParams(params: GRID_LAYOUT_Params) {
    ...
    private gridWidth: number; //获取的grid宽度
    ...
    computedColumnsTemplate() {
        ...
        let cols: string[] = [];
        if (this.gridInfo.weights !== undefined && this.gridInfo.spanCount !== undefined) { //当weights定义了，autoExpand失效
            if (this.gridInfo.weights.length == this.gridInfo.spanCount) { //当weights数组长度等于列数时，若数组内元素之和小于100，右侧则空出剩余部分，否则不进行空出
                ...
                if (element < 100) {
                    for (let j = 0; j < this.gridInfo.spanCount - this.gridInfo.weights.length; j++) {
                        cols.push(Math.round(100 - element) / (this.gridInfo.spanCount - this.gridInfo.weights.length) + 'fr'); //自动填充剩余占比
                    }
                    this.computedRightPadding(rightPadding);
                    this.columnsTemplate = cols.join(' ');
                }
            ...
            if (this.gridInfo.spanCount) {
                for (let i = 0; i < this.gridInfo.spanCount; i++) {
                    ...
                    cols.push(Math.round(element) + 'fr');
                }
                this.computedRightPadding(rightPadding);
                this.columnsTemplate = cols.join(' ');
    ...
    computedRowsTemplate() {
        ...
        if (this.gridInfo.spanCount) {
            for (let i = 0; i < this.vLayoutData.length; i++) {
                let afda: number | undefined = (this.vLayoutData[i] as layoutDataType).colsSpan;
                if (afda) {
                    if (afda > this.gridInfo.spanCount) { //如果合并列大于列数，合并项等于列数
                        afda = this.gridInfo.spanCount;
                    }
                    else if (afda < 1) { //如果合并列小于1，合并项等于1
                        (this.vLayoutData[i] as layoutDataType).colsSpan = 1;
                    }
                    else {
                        console.info(TAG + 'colsSpan = ' + (this.vLayoutData[i] as layoutDataType).colsSpan);
                    }
                    let colsSpan: number = 0;
                    if ((this.vLayoutData[i] as layoutDataType).colsSpan == undefined) {
                        colsSpan = 1;
                    }
                    else {
                        colsSpan = (this.vLayoutData[i] as layoutDataType).colsSpan as number;
                    }
                    sum = sum + colsSpan;
            ...
            let count: number = Math.ceil(sum / this.gridInfo.spanCount); //获得实际行数
            let rows: string[] = [];
            rows.push('1fr '.repeat(count)); //行数为1:1
            this.rowsTemplate = rows.join(' ');
        }
    }
    /*
    ...
    computedExtraHeight(rowCount: number) {
        if (this.gridInfo.vGap && this.layoutHeight && this.gridInfo.vGap >= 0 && typeof (this.gridInfo.vGap) == 'number') {

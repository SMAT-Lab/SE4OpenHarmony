...
import { AppendListener, CdToParentDirectoryListener, CurrentDirectoryListener, DeleteAllButSelfListener, DeleteAllListener, DeleteEmptyDirectoryListener, DeleteFileListener, DownloadDirListener, DownloadSingleListener, EnsureRemotePathListener, FeaturesListener, FileListListener, FileSizeListener, LastModifyListener, LoginListener, RenameFileListener, SetWorkingDirectoryListener, UploadDirListener, UploadSingleListener } from '../interfaces/NoTlsResultListener';
...
export default class NoTlsUtil {
    ...
    public async getTotalSize(cacheList: LinkedList<number>, localDir: string, listener?: UploadDirListener): Promise<number> {
        ...
        for (const file of files) {
            ...
            if (statErr) {
                if (listener)
                    listener.uploadDirErr(statErr);
                throw statErr;
            }
            if (!stats) {
                if (listener)
                    listener.uploadDirErr(new Error('获取本地文件信息失败，upload流程结束'));
                throw new Error('获取本地文件信息失败，upload流程结束');
            }
            if (stats.isFile()) {

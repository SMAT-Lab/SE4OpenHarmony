...
export default class Utils {
    ...
    public static comparisonObject(sourceObj, compareObj) {
        ...
        if (sourceType !== Utils.getType(compareObj))
            return false;
        ...
        if (sourceType !== "Array" && sourceType !== "Object" && sourceType !== "Set" && sourceType !== "Map") {
            ...
            if (sourceObj.length !== compareObj.length)
                return false;
            if (sourceObj.length === 0)
                return true;
            for (let i = 0; i < sourceObj.length; i++) {
                if (!Utils.comparisonObject(sourceObj[i], compareObj[i]))
                    return false;
            }
        }
            ...
            let key;
            if (sourceKeyList.length !== compareKeyList.length)
                return false;
            for (let i = 0; i < sourceKeyList.length; i++) {
                key = sourceKeyList[i];
                if (key !== compareKeyList[i])
                    return false;
                if (!Utils.comparisonObject(sourceObj[key], compareObj[key]))
                    return false;
            }
        }
        else if (sourceType === "Set" || sourceType === "Map") {
            // 把 Set Map 转为 Array
            if (!Utils.comparisonObject(Array.from(sourceObj), Array.from(compareObj)))
                return false;
        }
        return true;
    }
    ...
    public static emitsDifferentContext(expect: Function): void {
        let emitter = new EventEmitter<string, Object>(), pattern: string = '';
        function writerFunc() {
            pattern += this;
        }
    ...
    public static listenerArray(expect: Function): void {
        var e = new EventEmitter<string, Object>(), original;
        let bar: () => void = () => {
        };

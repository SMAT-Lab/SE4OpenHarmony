interface verticalViewPagerDemoActivityRe_Params {
    arr?: number[];
    index?: number;
    offsetY?: number;
    topSpaced?: number;
...
function generateId(): string {
    return "verticalViewPagerDemoActivityRe_" + ++__generate__Id;
}
/*
...
class verticalViewPagerDemoActivityRe extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        ...
        this.offsetFlag = 100;
        this.pageFlag = false;
        this.ScreenOffset = 6000;
        this.updateWithValueParams(params);
    }
    updateWithValueParams(params: verticalViewPagerDemoActivityRe_Params) {
    ...
    specificParam(item: any, parent = null) {
        ...
        if (item.i == this.index) {
            If.branchId(0);
            Flex.create();
            Flex.margin({ top: this.marginTop, bottom: this.marginBottom });
            Flex.width('100%');
    ...
    render() {
        ...
        if (earlierCreatedChild_2 == undefined) {
            View.create(new Toolbar("2", this, { title: 'Reverse Vertical ViewPager Demo', isBack: true }));
        }
        else {
            earlierCreatedChild_2.updateWithValueParams({
                title: 'Reverse Vertical ViewPager Demo', isBack: true
            });
...
loadDocument(new verticalViewPagerDemoActivityRe("1", undefined, {}));

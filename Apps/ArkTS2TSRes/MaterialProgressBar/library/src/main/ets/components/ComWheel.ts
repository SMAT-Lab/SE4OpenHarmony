interface ComWheel_Params {
    model?: Model;
}
let __generate__Id: number = 0;
function generateId(): string {
    return "ComWheel_" + ++__generate__Id;
}
export class ComWheel extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        super(compilerAssignedUniqueChildId, parent, localStorage);
        this.__model = new SynchedPropertyObjectTwoWay(params.model, this, "model");
        this.updateWithValueParams(params);
    }
    updateWithValueParams(params: ComWheel_Params) {
    }
    aboutToBeDeleted() {
        this.__model.aboutToBeDeleted();
        SubscriberManager.Get().delete(this.id());
    }
    public __model: SynchedPropertySimpleOneWay<Model>;
    get model() {
        return this.__model.get();
    }
    set model(newValue: Model) {
        this.__model.set(newValue);
    }
    render() {
        Column.create({ space: 5 });
        Grid.create();
        Grid.rowsTemplate('2fr 1fr');
        Grid.width(150);
        Grid.height(100);
        GridItem.create();
        GridItem.width(100);
        ...
        Stack.create();
        Path.create();
        Path.strokeWidth(this.model.barWidth);
        Path.fillOpacity(0);
        Path.stroke(this.model.barColor);
        Path.commands(this.model.svgPath);
        Path.width(50);
        Path.height(50);
        Path.zIndex(2);
        Path.create();
        Path.strokeWidth(this.model.rimWidth);
        Path.fillOpacity(0);
        Path.stroke(this.model.rimColor);
        Path.width(50);
        Path.height(50);
        Path.commands(this.model.rimSvgPath);
        Path.zIndex(0);
        Stack.pop();
        GridItem.pop();
        GridItem.create();
        Text.create(this.model.mProgress);
        Text.fontSize(15);

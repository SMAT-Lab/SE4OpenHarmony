interface MultipleRules_Params {
    message?: string;
    setValue?: string;
    typeValue?: string;
    resultStr?: string;
...
function generateId(): string {
    return "MultipleRules_" + ++__generate__Id;
}
/*
...
class MultipleRules extends View {
    ...
    updateWithValueParams(params: MultipleRules_Params) {
    ...
    ruleEngineExecute(): void {
        ...
        let rules: Rule[] = [new Rule1(), new Rule2()];
    ...
    render() {
        ...
        Text.create("目前transactionTotal 规则阻止值是小于：500，Card类型是：debit");
        Text.width("90%");
        Text.height(50);
        ...
        TextArea.create({ placeholder: "card：类型", controller: this.controller });
        TextArea.height(50);
        TextArea.width("100%");
        TextArea.margin({ top: 5, bottom: 5, left: 5, right: 5 });
        TextArea.onChange((value: string) => {
...
loadDocument(new MultipleRules("1", undefined, {}));

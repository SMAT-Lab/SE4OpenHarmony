...
function generateId(): string {
    return "compBirthday_" + ++__generate__Id;
}
export class compBirthday extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        super(compilerAssignedUniqueChildId, parent, localStorage);
        this.__text = new ObservedPropertySimple("KK's Birthday" // 之后改成传入的形式（AddPage）
        , this, "text");
        this.__date = new ObservedPropertySimple("2003-10-20" // 有没有日期格式的输入呢？
        , this, "date");
        this.__img = new ObservedPropertyObject($r("app.media.birthday") // 设置默认值（生日图标）
        // @State bgColor:     // 定义默认颜色
        , this, "img");
        this.updateWithValueParams(params);
    }
    updateWithValueParams(params: compBirthday_Params) {

interface compCountdown_Params {
    text?: string;
    date?: string;
    img?: Resource;
}
...
function generateId(): string {
    return "compCountdown_" + ++__generate__Id;
}
export class compCountdown extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        super(compilerAssignedUniqueChildId, parent, localStorage);
        this.__text = new ObservedPropertySimple("暑假开始" // 之后改成传入的形式（AddPage）
        , this, "text");
        this.__date = new ObservedPropertySimple("2023-6-30" // 有没有日期格式的输入呢？
        , this, "date");
        this.__img = new ObservedPropertyObject($r("app.media.countdown") // 设置默认值（生日图标）
        // @State bgColor:     // 定义默认颜色
        , this, "img");
        this.updateWithValueParams(params);
    }
    updateWithValueParams(params: compCountdown_Params) {

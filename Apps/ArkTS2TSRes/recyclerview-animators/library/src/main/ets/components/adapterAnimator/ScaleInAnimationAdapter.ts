interface ScaleInAnimationAdapter_Params {
    array?: Array<string>;
...
function generateId(): string {
    return "ScaleInAnimationAdapter_" + ++__generate__Id;
}
/*
...
export class ScaleInAnimationAdapter extends View {
    ...
    updateWithValueParams(params: ScaleInAnimationAdapter_Params) {
    ...
    aboutToAppear() {
        this.array.forEach((item: string) => {
            let temp = new RecyclerView.AnimationAdapterTypeData(0, item);
            this.list.push(temp);
    ...
    render() {
        ...
        List.onScrollIndex((start, end) => {
            ...
            this.animateList.forEach((item: any, index: number) => {
                if (index >= start && index <= end) {
                    let temp = new RecyclerView.AnimationAdapterTypeData(1, item.value);
        ...
        ForEach.create("2", this, ObservedObject.GetRawObject(this.list), (item: any, index: number) => {
            ...
            ListItem.scale({
                ...
                z: this.animateList[index].animationAdapterKey
            });

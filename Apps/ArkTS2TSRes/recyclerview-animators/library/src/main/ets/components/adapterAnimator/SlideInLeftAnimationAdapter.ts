interface SlideInLeftAnimationAdapter_Params {
    array?: Array<string>;
...
function generateId(): string {
    return "SlideInLeftAnimationAdapter_" + ++__generate__Id;
}
/*
...
export class SlideInLeftAnimationAdapter extends View {
    ...
    updateWithValueParams(params: SlideInLeftAnimationAdapter_Params) {
    ...
    aboutToAppear() {
        this.array.forEach((item: string) => {
            let temp = new RecyclerView.AnimationAdapterTypeData(-this.controller.mWidth, item);
            this.list.push(temp);
    ...
    render() {
            ...
            this.animateList.forEach((item: any, index: number) => {
                if (index >= start && index <= end) {
                    ...
                    if (this.controller.isFirstOnly) {
                        let temp: RecyclerView.AnimationAdapterTypeData = new RecyclerView.AnimationAdapterTypeData(-this.controller.mWidth, item.value);
                        this.animateList.splice(index, 1, temp);
                    }
                }
            });
        ...
        ForEach.create("2", this, ObservedObject.GetRawObject(this.list), (item: any, index: number) => {
            ...
            ListItem.width('100%');
            ListItem.translate({
                x: this.animateList[index].animationAdapterKey
            });

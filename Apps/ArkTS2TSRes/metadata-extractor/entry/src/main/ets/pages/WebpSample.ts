interface WebpSample_Params {
    isVisibility?: Visibility;
    data?: Array<string>;
}
let __generate__Id: number = 0;
function generateId(): string {
    return "WebpSample_" + ++__generate__Id;
}
/*
...
class WebpSample extends View {
    ...
    updateWithValueParams(params: WebpSample_Params) {
    ...
    private getMetaData() {
        ...
        let pathWebp = fileDir.concat("/metadata.WEBP");
        try {
            let metadata: Metadata = ImageMetadataReader.readMetadata(pathWebp);
    ...
    render() {
        ...
        Text.pop();
        Text.create("资源路径:/resource/metadata.WEBP");
        Text.fontSize(22);
        Text.width('100%');
        ...
        Text.pop();
        Column.pop();
        Button.createWithLabel("显示WEBP文件的元数据", { type: ButtonType.Normal, stateEffect: true });
...
loadDocument(new WebpSample("1", undefined, {}));

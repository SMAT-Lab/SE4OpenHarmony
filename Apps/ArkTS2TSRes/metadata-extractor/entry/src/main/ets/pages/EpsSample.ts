interface BmpSample_Params {
    isVisibility?: Visibility;
    data?: Array<string>;
}
let __generate__Id: number = 0;
function generateId(): string {
    return "EpsSample_" + ++__generate__Id;
}
/*
...
class BmpSample extends View {
    ...
    updateWithValueParams(params: BmpSample_Params) {
    ...
    private getMetaData() {
        ...
        let pathAvi = fileDir.concat("/adobeJpeg1.eps");
        try {
            let metadata: Metadata = ImageMetadataReader.readMetadata(pathAvi);
            ...
            console.debug("metadata esp data:" + JSON.stringify(this.data));
        }
        catch (err) {
    ...
    render() {
        ...
        Text.pop();
        Text.create("资源路径:/resource/adobeJpeg1.eps");
        Text.fontSize(22);
        Text.width('100%');
        ...
        Text.pop();
        Column.pop();
        Button.createWithLabel("显示Eps文件的元数据", { type: ButtonType.Normal, stateEffect: true });
...
loadDocument(new BmpSample("1", undefined, {}));

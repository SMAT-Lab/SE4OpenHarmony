interface GifSample_Params {
    isVisibility?: Visibility;
    data?: Array<string>;
}
let __generate__Id: number = 0;
function generateId(): string {
    return "GifSample_" + ++__generate__Id;
}
/*
...
class GifSample extends View {
    ...
    updateWithValueParams(params: GifSample_Params) {
    ...
    private getMetaData() {
        ...
        let pathGif = fileDir.concat("/metadata.gif");
        try {
            let metadata: Metadata = ImageMetadataReader.readMetadata(pathGif);
            ...
            console.info("metadata gif data:" + this.data);
        }
        catch (err) {
    ...
    render() {
        ...
        Text.pop();
        Text.create("资源路径:/resource/metadata.gif");
        Text.fontSize(22);
        Text.width('100%');
        ...
        Text.pop();
        Column.pop();
        Button.createWithLabel("显示GIF文件的元数据", { type: ButtonType.Normal, stateEffect: true });
...
loadDocument(new GifSample("1", undefined, {}));

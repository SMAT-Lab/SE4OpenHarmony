interface AudioClipPage_Params {
    btnText?: string;
    btnText2?: string;
    imageWidth?: number;
    imageHeight?: number;
...
function generateId(): string {
    return "audioClipPage_" + ++__generate__Id;
}
/*
...
class AudioClipPage extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        super(compilerAssignedUniqueChildId, parent, localStorage);
        this.__btnText = new ObservedPropertySimple("音频裁剪点击执行", this, "btnText");
        this.__btnText2 = new ObservedPropertySimple("音频裁剪点击执行", this, "btnText2");
    ...
    updateWithValueParams(params: AudioClipPage_Params) {
    ...
    render() {
        ...
        Button.pop();
        Button.createWithLabel(this.btnText2 + "_15", { type: ButtonType.Normal, stateEffect: true });
        Button.borderRadius(8);
        Button.backgroundColor(0x317aff);
        Button.width(250);
        Button.margin({ top: 15 });
     ...
     * 音频裁剪
     */
    private audioClipTwo() {
        let getLocalDirPath = getContext(this).cacheDir + "/";
        let that = this;
        let sTime = "00:00:02";
        let eTime = "00:00:05";
        let sourcePath = getLocalDirPath + "testsound.wav";
        let outPath = getLocalDirPath + "clipout15.wav";
        let callBack: ICallBack = {
            callBackResult(code: number) {
                that.btnText2 = "音频裁剪点击执行";
                that.imageWidth = 0;
                that.imageHeight = 0;
                if (code == 0) {
                    AlertDialog.show({ message: '执行成功' });
...
loadDocument(new AudioClipPage("1", undefined, {}));

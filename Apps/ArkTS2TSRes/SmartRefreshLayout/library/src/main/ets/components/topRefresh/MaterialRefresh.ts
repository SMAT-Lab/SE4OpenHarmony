interface Material_Params {
    ...
    refreshData?: MaterialSimple.Model;
    refresh?: boolean;
    refreshState?: boolean;
    dragRefreshData?: MaterialSimple.Model;
    dragPathCmd?: string;
    init?: boolean;
    lastYOffset?: number;
    intervalID?: number;
...
function generateId(): string {
    return "MaterialRefresh_" + ++__generate__Id;
}
/*
...
export class Material extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        ...
        this.__shadowCommands = new ObservedPropertySimple('', this, "shadowCommands");
        this.screenWidth = display.getDefaultDisplaySync().width;
        this.multiple3 = 3;
        this.updateWithValueParams(params);
    }
    updateWithValueParams(params: Material_Params) {
        ...
        if (params.shadowCommands !== undefined) {
            this.shadowCommands = params.shadowCommands;
        }
    ...
    aboutToBeDeleted() {
        ...
        this.__shadowCommands.aboutToBeDeleted();
    ...
    private __refreshData: ObservedPropertyObject<MaterialSimple.Model>;
    get refreshData() {
        return this.__refreshData.get();
    }
    set refreshData(newValue: MaterialSimple.Model) {
    ...
    //拖拽变量
    private dragRefreshData: MaterialSimple.Model;
    ...
    private __shadowCommands: ObservedPropertySimple<string>;
    get shadowCommands() {
        return this.__shadowCommands.get();
    }
    set shadowCommands(newValue: string) {
        this.__shadowCommands.set(newValue);
    }
    private screenWidth: number;
    private multiple3: number;
    aboutToAppear() {
        this.model.headerHeight = this.model.initHeaderHeight;
        this.refreshColor = this.model.progressBackgroundColorSchemeResource;
    ...
    shadowCommand() {
        if (this.model.headerHeight < this.model.initHeaderHeight || this.model.refreshState == SmartRefresh.REFRESHSTATE.REFRESHING) {
            let height = this.model.headerHeight * this.multiple3;
            this.shadowCommands = "M0 0 L0 " + height
                + " L " + this.screenWidth + " " + height + " L " + this.screenWidth + " 0 Z";
        }
        if (this.model.headerHeight >= this.model.initHeaderHeight && this.model.refreshState == SmartRefresh.REFRESHSTATE.TOREFRESH) {
            let height = this.model.initHeaderHeight * this.multiple3;
            this.shadowCommands = "M0 0 L0 " + height
                + "C0 " + height + " " + (this.screenWidth / 2) + " " + (height + (this.model.headerHeight - this.model.initHeaderHeight) * 0.9 * this.multiple3)
    ...
    draw(): void {
        this.model.headerRefreshId = setInterval(() => {
            if (this.model.refreshState == SmartRefresh.REFRESHSTATE.TOREFRESH) {
                let increase_distanceToAngle: number = 360 * this.model.headerHeight / 150;
                let reduce_distanceToAngle: number = 135 * this.model.headerHeight / 150;
                if (increase_distanceToAngle > (reduce_distanceToAngle + 300)) {
                    increase_distanceToAngle = reduce_distanceToAngle + 300;
                }
    ...
    private onDraw(): void {
        ...
        if (this.refreshData.isSpinning) {
            ...
            if (this.refreshData.barExtraLength <= this.latestAngle) {
                ...
                this.latestAngle = this.refreshData.barExtraLength;
                if (this.flag) {
                    this.refreshData.setBarColor(this.refreshColor[this.latestIndex % this.refreshColor.length]);
                    this.latestIndex += 1;
                    this.flag = false;
                }
            }
    ...
    render() {
        ...
        if (this.model.refreshState == SmartRefresh.REFRESHSTATE.REFRESHING) { //松开过后的刷新样式
            ...
            //              .fill(this.model.getBackgroundShadowColor())
            //              .margin({top: 40, left: 153})
            Path.commands(this.shadowCommands);
            //            Circle({ width:  65, height: 65 })
            //              .fillOpacity(3)
            //              .stroke(Color.Gray)
            //              .strokeWidth(3)
            //              .fill(this.model.getBackgroundShadowColor())
            //              .margin({top: 40, left: 153})
            Path.fill(this.model.accentColor);
            Path.create();
            Path.strokeWidth(3);
            ...
            Path.margin({ left: (this.screenWidth / 6), top: ((this.model.headerHeight / 2 - 70) * 3 + 45) });
            Path.create();
            Path.strokeWidth(5);
            Path.fillOpacity(0);
            ...
            Path.height("100%");
            Path.zIndex(2);
            Path.margin({ left: (this.screenWidth / 6) - 35, top: ((this.model.headerHeight / 2 - 70) * 3 + 60) });
            Path.opacity(1);
            Stack.pop();
        }
            ...
            If.branchId(1);
            If.create();
            if (this.model.headerHeight > (this.model.initHeaderHeight * 3 / 4)) {
                If.branchId(0);
                Stack.create({ alignContent: Alignment.Start });
                Path.create();
                Path.commands(this.shadowCommands);
                ...
                Path.strokeWidth(3);
                Path.stroke(4);
                Path.commands("M0 70 A 80 80, 0, 1, 1, 0 69.99");
                Path.width(50);
                Path.height(50);
                Path.margin({ left: (this.screenWidth / 6),
                    top: this.model.headerHeight < this.model.initHeaderHeight ? this.model.headerHeight - this.model.initHeaderHeight
                        : ((this.model.headerHeight - this.model.initHeaderHeight) < 48 ? (this.model.headerHeight - this.model.initHeaderHeight)
                            : 48) });
                Path.fill(Color.White);
                Path.create();
                ...
                Path.height(50);
                Path.zIndex(2);
                Path.margin({ top: this.model.headerHeight < this.model.initHeaderHeight ? this.model.headerHeight - this.model.initHeaderHeight
                        : ((this.model.headerHeight - this.model.initHeaderHeight) < 48 ? (this.model.headerHeight - this.model.initHeaderHeight)
                            : 48),
                    left: 185 });
                Path.opacity(1);
...
namespace MaterialSimple {
    export class Model {
        ...
        init() {
            ...
            this.startX = 0; //this.circleRadius

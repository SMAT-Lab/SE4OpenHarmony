interface DropBoxBottomRefresh_Params {
...
function generateId(): string {
    return "DropBoxBottomRefresh_" + ++__generate__Id;
}
/*
 ...
 */
import SmartRefreshForDropBoxSample from "../topRefresh/SmartRefreshForDropBoxSample";
class Drawable3PathsParam {
    index: number = 0;
    content: string = "";
}
export class DropBoxBottomRefresh extends View {
    ...
    updateWithValueParams(params: DropBoxBottomRefresh_Params) {
    ...
    aboutToAppear() {
        this.model.setRefreshBottomCallback((): void => this.draw());
    }
    draw() {
    ...
    boxPath(str: string, color: number, parent = null) {
        ...
        Path.fill(color);
        Path.width('100%');
        Path.height(this.model.footerHeight);
        Path.scale({
            x: 0.6,
            y: 0.6,
            centerX: 0,
    ...
    render() {
        ...
        if (this.model.refreshState == SmartRefreshForDropBoxSample.REFRESHSTATE.REFRESHING) { //松开过后的刷新样式
            ...
            ForEach.create("2", this, ObservedObject.GetRawObject(this.drawable1Paths), (item: Drawable3PathsParam) => {
                ...
                Path.translate({
                    y: (this.model.footerHeight - this.model.initFooterHeight) / 3 + 100 + (this.time < 0 ? this.time * 4 : 0)
                });
                Path.height(this.model.footerHeight);
            ...
            ForEach.create("3", this, ObservedObject.GetRawObject(this.drawable1Paths), (item: Drawable3PathsParam) => {
                ...
                Path.translate({
                    y: (this.model.footerHeight - this.model.initFooterHeight) / 3 + 100 + ((this.time > 100 ? (this.time % 120 > 100 && this.time % 120 < 120 ? (this.time % 120 - 120) * 4 : 0) : 1000))
                });
                Path.height(this.model.footerHeight);
            }, (item: Drawable3PathsParam) => item.index.toString());
            //红包
            ...
            ForEach.create("4", this, ObservedObject.GetRawObject(this.drawable2Paths), (item: Drawable3PathsParam) => {
                ...
                Path.translate({
                    y: (this.model.footerHeight - this.model.initFooterHeight) / 1.25 + 100 + ((this.time > 20 ? (this.time < 40 ? (this.time - 40) * 4 : 0) : 1000))
                });
            }, (item: Drawable3PathsParam) => item.index.toString());
            //文件
            ForEach.pop();
            ...
            ForEach.create("5", this, ObservedObject.GetRawObject(this.drawable2Paths), (item: Drawable3PathsParam) => {
                ...
                Path.translate({
                    y: (this.model.footerHeight - this.model.initFooterHeight) / 1.25 + 100 + (this.time > 140 ? (this.time % 120 > 20 && this.time % 120 < 40 ? (this.time % 120 - 40) * 4 : 0) : 1000)
                });
            }, (item: Drawable3PathsParam) => item.index.toString());
            //文件
            ForEach.pop();
            ...
            ForEach.create("6", this, ObservedObject.GetRawObject(this.drawable3Paths), (item: Drawable3PathsParam) => {
                ...
                Path.translate({
                    y: (this.model.footerHeight - this.model.initFooterHeight) / 4 + 100 + ((this.time > 60 ? (this.time < 80 ? (this.time - 80) * 4 : 0) : 1000))
                });
            }, (item: Drawable3PathsParam) => item.index.toString());
            //音乐
            ForEach.pop();
            ...
            ForEach.create("7", this, ObservedObject.GetRawObject(this.drawable3Paths), (item: Drawable3PathsParam) => {
                ...
                Path.translate({
                    y: (this.model.footerHeight - this.model.initFooterHeight) / 4 + 100 + (this.time > 180 ? (this.time % 120 > 60 && this.time % 120 < 80 ? (this.time % 120 - 80) * 4 : 0) : 1000)
                });
            }, (item: Drawable3PathsParam) => item.index.toString());
            //音乐
            ForEach.pop();
            ...
            this.boxPath("M0 149L85 198L170 149Q170 99 170 49L85 0L0 49Q0 99 0 149Z", this.boxColor[0], this);
            this.boxPath("M0 49L85 0L25 -30L-60 19ZM170 49L85 0L145 -30L230 19ZM170 49L85 99L145 129L230 79ZM0 49L85 99L25 129L-60 79Z", this.boxColor[1], this);
            Stack.pop();
        }
        If.pop();
        Flex.pop();

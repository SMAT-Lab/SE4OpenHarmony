interface ClassicsBottom_Params {
    model?: SmartRefresh.Model;
    classicsSimple?: ClassicsBottomModel.Model;
    refresh?: boolean;
...
function generateId(): string {
    return "ClassicsBottomRefresh_" + ++__generate__Id;
}
/*
...
export class ClassicsBottom extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        super(compilerAssignedUniqueChildId, parent, localStorage);
        this.__model = new SynchedPropertyObjectTwoWay(params.model, this, "model");
        this.__classicsSimple = new ObservedPropertyObject(new ClassicsBottomModel.Model(), this, "classicsSimple");
        ...
        this.intervalID = 0;
        this.information = '';
        this.arrowDown = 'M 25 20 L25 120 L0 85 L25 120 L50 85 L25 120';
    ...
    updateWithValueParams(params: ClassicsBottom_Params) {
    ...
    private __classicsSimple: ObservedPropertyObject<ClassicsBottomModel.Model>;
    get classicsSimple() {
        return this.__classicsSimple.get();
    }
    set classicsSimple(newValue: ClassicsBottomModel.Model) {
    ...
    render() {
        ...
        if (this.model.refreshState == SmartRefresh.REFRESHSTATE.REFRESHING) { //松开过后的刷新样式
            ...
            Path.width(100);
            Path.height(200);
            Path.stroke(this.model.getClassicsBottomAccentColor());
            Path.margin({ left: 50, top: this.model.footerHeight / 2 });
            Column.create();
            Column.width("70%");
            Column.alignItems(HorizontalAlign.Center);
            Column.margin({ left: 80, top: (this.model.headerHeight - 150) / 2 });
            Row.create();
            Text.create(this.model.classicsBottomRefreshRefreshing);
            Text.fontSize(this.model.classicsBottomTextSizeTitle);
            Text.height(50);
            Text.margin({ top: 100 + this.calculateTopPosition(), left: this.model.classicsBottomDrawableMarginRight });
            Text.fontColor(this.model.getClassicsBottomAccentColor());
            Text.pop();
            Row.pop();
            If.create();
            if (this.model.getTimeShowState()) {
                ...
                Text.fontSize(this.model.classicsBottomTextSizeTime);
                Text.height(50);
                Text.align(Alignment.Center);
                Text.margin({ top: this.calculateTopPosition(), left: this.model.classicsBottomDrawableMarginRight });
                Text.fontColor(this.model.getClassicsBottomAccentColor());
                Text.pop();
                Row.pop();
            }
            ...
            Column.margin({ left: 80, top: (this.model.headerHeight - 150) / 2 });
            Row.create();
            Text.create(this.model.headerHeight < this.model.initHeaderHeight ? this.model.classicsBottomRefreshPullDown : this.model.classicsBottomRefreshRelease);
            Text.height(50);
            Text.fontSize(this.model.classicsBottomTextSizeTitle);
            Text.fontColor(this.model.getClassicsBottomAccentColor());
            Text.margin({ top: 100 + this.calculateTopPosition(), left: this.model.classicsBottomDrawableMarginRight });
            ...
            Text.margin({ top: 20, left: px2vp(this.screenWidth / 4) });
            Text.pop();
            Row.pop();
        }
        If.pop();

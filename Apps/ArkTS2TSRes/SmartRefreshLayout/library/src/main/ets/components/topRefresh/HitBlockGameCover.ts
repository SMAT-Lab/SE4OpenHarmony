interface HitBlockGameCover_Params {
    modelCover?: SmartRefreshForGame.Model;
    gameState?: boolean;
    maxHeight?: number;
    downY?: number;
...
function generateId(): string {
    return "HitBlockGameCover_" + ++__generate__Id;
}
/*
...
export class HitBlockGameCover extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        super(compilerAssignedUniqueChildId, parent, localStorage);
        this.__modelCover = new SynchedPropertyObjectTwoWay(params.modelCover, this, "modelCover");
        ...
        this.__tipText = new ObservedPropertySimple(this.modelCover.maskTextTopPull, this, "tipText");
        this.updateWithValueParams(params);
        this.declareWatch("gameState", this.onGameChange);
        this.declareWatch("downY", this.onYChange);
    }
    updateWithValueParams(params: HitBlockGameCover_Params) {
    ...
    aboutToBeDeleted() {
        this.__modelCover.aboutToBeDeleted();
    ...
    private __modelCover: SynchedPropertySimpleOneWay<SmartRefreshForGame.Model>;
    get modelCover() {
        return this.__modelCover.get();
    }
    set modelCover(newValue: SmartRefreshForGame.Model) {
        this.__modelCover.set(newValue);
    }
    ...
    private onYChange() {
        if (this.downY < this.maxHeight) {
            this.tipText = this.modelCover.maskTextTopPull;
        }
        else {
            this.tipText = this.modelCover.maskTextTopRelease;
        }
    }
    render() {
        ...
        Text.fontSize(px2fp(this.modelCover.maskTextSizeTop));
        ...
        Context.animation(null);
        Text.create(this.modelCover.maskTextBottom);
        Text.fontSize(px2fp(this.modelCover.maskTextSizeBottom));
        Text.fontColor(this.textColor);
        Text.pop();
        Flex.pop();
        Flex.pop();

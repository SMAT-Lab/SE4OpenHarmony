 ...
 */
import { Constant, Flag, Folder, GlobalContext, MailLogger, Message, Properties, RecipientType, Store } from '@ohos/mail';
...
class Pop extends View {
    constructor(compilerAssignedUniqueChildId, parent, params, localStorage) {
        ...
        this.__hostReceive = new ObservedPropertySimple("pop.qq.com", this, "hostReceive");
        this.__portReceive = new ObservedPropertySimple(110, this, "portReceive");
        this.readMsgIndex = 1;
    ...
    updateWithValueParams(params: Pop_Params) {
        ...
        if (params.popHost !== undefined) {
            this.popHost = params.popHost;
        }
                            ...
                            for (let i = start; i < messages.length; i++) {
                                await new Promise<string>((resolve, reject) => {
                                    messages[i].getAllHeaders((success: boolean, result: any) => {
                                        if (success) {
                                            this.msgList += i + 1 + ' 发件人：' + result.getFrom()[0] + Constant.LINEFEED + Constant.LINEFEED
                            ...
                            if (!err) {
                                ...
                                if (messages.length > 0 && messages.length >= index) {
                                    ...
                                    try {
                                        let message = await msg.syncGetContent();
                                        let result = message as Message;
                                        parseResult = '发件人:  ' + result.getFrom()[0] + "\r\n\r\n"
                                            + '主题:  ' + result.getSubject() + "\r\n\r\n"
                                            + '收件人:  ' + result.getRecipients(RecipientType.TO) + "\r\n\r\n"
                                            + '正文:  ' + result.getText() + "\r\n\r\n"
                                            + 'HTML:  ' + result.getHtml() + "\r\n\r\n";
                                        ...
                                        if (!!files) {
                                            let length = files.length;
                                            for (let i = 0; i < length; i++) {
                                                parseResult += "附件  " + (i + 1) + ": " + files[i].getFileName() + "\r\n\r\n"
                                                    + "    Content-Transfer-Encoding:" + files[i].getEncoding() + "\r\n\r\n"
                                                    + "    Content-Type:" + files[i].getMimeType() + "\r\n\r\n"
                                                    + "    附件内容长度:" + files[i].getData().length + "\r\n\r\n";
                                            }
...
loadDocument(new Pop("1", undefined, {}));

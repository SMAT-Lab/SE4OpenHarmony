 ...
 * @return 名称列表
 */
export function getLunarDays(maxDay: number): string[] {
    let days: string[] = [];
    MaxDay.forEach(item => {
        if (maxDay > 0) {
            if (item <= maxDay) {
                days.push(getChinaDate(Number(item)));
            }
        }
    });
    return days;

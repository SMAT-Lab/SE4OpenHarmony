...
function generateId(): string {
    return "GridViewDemo_" + ++__generate__Id;
}
/*
...
class GridViewDemo extends View {
    ...
    updateWithValueParams(params: GridViewDemo_Params) {
    ...
    verticallyView(leftColorName: string, rightColorName: string, leftColor: string, rightColor: string, parent = null) {
        Row.create({ space: 10 });
        Row.width("100%");
        Row.height("25%");
        Text.create('' + leftColorName);
        Text.height("100%");
        Text.width("50%");
        Text.fontSize(30);
        Text.textAlign(TextAlign.Center);
        Text.backgroundColor(leftColor);
        Text.onClick((event?: ClickEvent) => {
            prompt.showToast({ message: "Tapped on: " + leftColorName, duration: 2000 });
        });
        Gesture.create(GesturePriority.Low);
        LongPressGesture.create();
        LongPressGesture.onAction((event?: GestureEvent) => {
            prompt.showToast({ message: "Long-tapped on: " + leftColorName, duration: 2000 });
        });
        LongPressGesture.pop();
        Gesture.pop();
        Text.pop();
        Text.create('' + rightColorName);
        Text.height("100%");
        Text.width("50%");
        Text.fontSize(30);
        Text.textAlign(TextAlign.Center);
        Text.backgroundColor(rightColor);
        Text.onClick((event?: ClickEvent) => {
            prompt.showToast({ message: "Tapped on: " + rightColorName, duration: 2000 });
        });
        Gesture.create(GesturePriority.Low);
        LongPressGesture.create();
        LongPressGesture.onAction((event?: GestureEvent) => {
            prompt.showToast({ message: "Long-tapped on: " + rightColorName, duration: 2000 });
        });
        LongPressGesture.pop();
        Gesture.pop();
        Text.pop();
    ...
    SpecificChild(parent = null) {
        ...
        Column.width('100%');
        this.verticallyView(this.colorName[0], this.colorName[1], this.color[0], this.color[1], this);
        this.verticallyView(this.colorName[2], this.colorName[3], this.color[2], this.color[3], this);
        this.verticallyView(this.colorName[4], this.colorName[5], this.color[4], this.color[5], this);
        this.verticallyView(this.colorName[6], this.colorName[7], this.color[6], this.color[7], this);
        this.verticallyView(this.colorName[8], this.colorName[9], this.color[8], this.color[9], this);
        this.verticallyView(this.colorName[10], "", this.color[10], "", this);
        Column.pop();
    }
    render() {
        ...
        Text.create("GridView Over-Scroll Demo");
        Text.fontSize(38);
        Text.maxLines(1);
...
loadDocument(new GridViewDemo("1", undefined, {}));
